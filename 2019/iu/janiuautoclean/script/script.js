...
$(window).on('scroll', function () {
	...
	if ($(window).scrollTop() > scrollHeight) {
		$('.problem .message-box').addClass("on");
	}
...
function trdclean() {
	$(".banner ul").addClass("nomail")
	$("li.default").removeClass("hide")
	$("li.first").remove();
}
function soldOut() {
	...
	$(".price b").html("16")
	$(".buybtn").not('.buybtn-pop').attr("href", "https://www.iobit.com/buy.php?product=iu113pc1677&ref=en-iu3pc1677soldoutautoclean" + refStr + "&aff=" + aff + "&refs=purchase_iu&tw=" + tw);
	$(".buybtn").not('.buybtn-pop').attr("onclick", "ga('send', 'event', 'iubuysold', 'buy', 'iupurchase-autoclean');");
	$(".countdown").removeClass("twinkle");
}
...
function packsInit() {
	var date = new Date();
	var cName = "eniuauto" + date.getDate();
...
$(function () {
	packsInit();
	timeInit();
	if (MApp(2.2).url.getParameters('to') == 'trdclean') {
		trdclean();
	}
});