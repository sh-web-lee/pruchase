...
function soldOut() {
	...
	$(".between-buy h2").html("Jetzt vorbestellen, um IObit Uninstaller 9 PRO zu genießen");
	$(".between-buy .price .change").remove();
	$(".price").find("strong").html("€<big>18</big>,99€");
	$(".buybtn").attr({
		"href": 'http://www.iobit.com/buy.php?product=de-iu9preorder1899&ref=de_iu9preorder1899exp' + refStr + '&aff=' + aff + '&refs=de_purchase_iu&tw=' + tw,
		"onclick": "ga('send', 'event', 'iu3pcbuy', 'buy', 'iu9predorderexp-de');"
	});
}
// soldOut();
...
$(function() {
	...
	var index = 0;
	var picTimer;
	var listCount=$(".right-message:eq(0) li").not(".normal li").length;
	if(listCount>0) {
		if (listCount <= 2) {
