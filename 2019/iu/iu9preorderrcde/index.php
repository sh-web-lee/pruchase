...
if ($_GET['action'] == 'getPacks') {
	$date = $_GET['d'];
	$c_name = 'deiurcjune_p' . $date;
	$c_name_t='deiurcjune_t' . $date;
		...
		<div class="wrapper">
			...
			<div class="clearfix">
				...
				<div class="right fr">
					...
					<p>Deinstalliere alles ohne Spuren zu hinterlassen</p>
					<?php else:?>
					<p class="data">
						<?php if(intval($transData->softwarehealth->plugin)>0):?>
						1-Klick um <strong><?php echo intval($transData->softwarehealth->plugin);?></strong> gefährliche Plug-In(s) zu entfernen <br>
						<?php endif;?>
						<?php if(intval($transData->softwarehealth->software)>0):?>
							<?php echo (intval($transData->softwarehealth->plugin)>0)?'und':'';?> <strong><?php echo intval($transData->softwarehealth->software);?></strong> veraltete Programme auf ihrem PC zu entfernen
						<?php endif;?>
					</p>
					<?php endif;?>
					<ul class="price clearfix">
						...
						<li>
							<a class="buybtn"
							   href="https://www.iobit.com/buy.php?product=de-iu9preorder&ref=de_iu9preorderrc<?php echo $refStr;?>&aff=<?php echo $_GET['aff'];?>&refs=de_purchase_iu"
							   onclick="ga('send', 'event', 'iubuy', 'buy', 'iu9predorderrc-de');">
							  Jetzt vorbestellen
							</a>
						</li>
					</ul> 
				...
				<div class="clearfix">
					...
					<div class="right fr">
						<ul class="feature-list">
							...
							<li><i class="all-icons">√</i> Technischer premium Support rund um die Uhr auf Anfrage</li>
						</ul>
					</div>
				</div>
