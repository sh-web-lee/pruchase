...
function soldOut() {
  ...
  flipNum(0, 4);
  $(".banner").find(".change").html("As reservas de pré-venda se esgotaram, não perca o desconto novamente!");
  $(".banner .left, .floatlayer .imgbox").append('<span class="discount">55% OFF</span>');
  setTimeout('$(".discount").addClass("on")', 300);
  $(".floatlayer").find(".one").html('<p>As reservas de pré-venda se <br> esgotaram, não perca novamente!</p>');
  $(".feature .price").find("p").remove();
  $(".feature .price div.fl").css({"marginTop": 55});
  $(".price").find("strong").html("R$<big>59</big>,99");
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=pt-iu93pc5999&ref=pt_iu9preorder5999useron' + refStr + '&aff=' + aff + '&refs=purchase_iu&tw=' + tw,
    "onclick": "ga('send', 'event', 'iubuy', 'buy', 'iu9predorderiusoldout-pt');"
  });
}
