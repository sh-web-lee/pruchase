...
function soldOut() {
	...
	$(".banner,.between-buy,.floatlayer").removeClass("sold").addClass("sold");
	flipNum(0, 4);
	$(".banner").find(".change").html("$0 preorder reservations are full, don't miss out this discount again!");
	$(".banner .left, .floatlayer .imgbox").append('<span class="discount">55% OFF</span>');
	setTimeout('$(".discount").addClass("on")', 300);
	$(".floatlayer").find(".one").html('<p>Pre-order now to be the first one <br>to enjoy IObit Uninstaller 9 PRO</p>');
	$(".between-buy .price").find("p").remove();
	$(".between-buy h2").html("Pre-order Now to Enjoy IObit Uninstaller 9 PRO");
	$(".price").find("strong").html("$<big>14</big>.77");
	$(".buybtn").attr({
		"href": 'https://www.iobit.com/buy.php?product=iu9preorder1477&ref=iu9preorder1477exp'+refStr+'&aff=' + aff + '&refs=purchase_iu&tw=' + tw,
		"onclick": "ga('send', 'event', 'iu3pcbuy', 'buy', 'iu9predorderexp');"
	});
}
// soldOut();
