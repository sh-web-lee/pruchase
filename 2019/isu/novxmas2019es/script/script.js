...
function soldOut() {
	...
	$(".banner .right-message h2").html("<span>Lo sentimos, los regalos se han acabado.</span> De todas formas, todavía tienes la oportunidad de conseguir un <strong>55% de descuento</strong> en");
	$(".banner .right-message .progress").html("<h3>IObit Software Updater 4 PRO</h3> <p>1 año de suscripción / 3 PCs</p>");
	$(".banner .right-message ul span:eq(0)").html("29,99€");
	$(".banner .right-message ul span:eq(1)").html("55%");
	$(".banner .right-message ul span:eq(2)").html("17€");
	$(".floatlayer .number").html("<h2>Actualiza tus programas <br>Corrige las brechas de seguridad </h2>");
	$(".floatlayer .price del").html("29,99€");
	$(".feature .price span >span").remove();
	$(".floatlayer .wrapper").append("<p class='year'>1 año de suscripción / 3 PCs</p>");
	$("span.snow02").fadeOut();
	$(".buybtn").attr({
		"href": 'https://www.iobit.com/buy.php?product=es-isu43pc1299&ref=es_isu3pc2104' + refStr + '&refs=es_purchase_isu&tw=' + tw,
		"onclick": "ga('send', 'event', 'isubuy', 'buy', 'isupurchase1912-es');"
	});
}
// soldOut();
...
function getReTime() {
	var packsNumTotal = cGet('isunovxmas2020_t');
	if (packsNumTotal > 1) {
