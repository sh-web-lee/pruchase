...
function soldOut() {
	...
	$(".banner .right-message h2").html("<span>Mi dispiace, i regali gratis sono esauriti</span> Non perdere quest’altra opportunità del <strong>55% di sconto</strong>!");
	$(".banner .right-message .progress").html("<h3>IObit Software Updater 4 PRO</h3> <p>1 Anno Abbonamento / 3 PC</p>");
  $(".banner .right-message ul span:eq(0)").html("€29,99");
  $(".banner .right-message ul span:eq(1)").html("55%");
  $(".banner .right-message ul span:eq(2)").html("€17,00");
	$(".floatlayer .number").html("<h2>Aggiorna I tuoi programmi <br> Ripara le falle di sicurezza ora</h2>");
	$(".floatlayer .price del").html("€29,99");
	$(".feature .price span >span").remove();
	$(".floatlayer .wrapper").append("<p class='year'>1 Anno Abbonamento / 3 PC</p>");

	$(".buybtn").attr({
		"href": 'https://www.iobit.com/buy.php?product=it-isu43pc1299&ref=it_isu43pc1299purchase2004'+refStr+'&aff='+ aff +'&refs=it_purchase_isu&tw=' + tw,
		"onclick": "ga('send', 'event', 'isubuy', 'buy', 'isupurchase1905ex-it');"
	});
}
