...
function soldOut() {
  ...
  $(".change-title").html("<p>Oops, 65% discount SOLD OUT!</p><h2>However, <span>55% discount</span> is still available.</h2>");
  $(".feature h2").find("span").html("55%");
  $(".change-price").html('$<big>12</big>.97');
  $(".popbox .change-price").html('$<big>8</big>.99');
  $(".buybtn.exbtn.soldout").attr({
    "href": "https://www.iobit.com/buy.php?product=isu3pc1297&ref=xr_isu1297" + refStr+"&refs=purchase_isux",
    "onclick": "ga('send', 'event', 'isubuysold', 'buy', 'isupurchase-xmas');",
  });
}
...
function mask(scroll) {
  ...
  maskTimeiu = cGet('maskisu_t');

  if ((scroll == 1) && (_maskTimetwoiu - maskTimeiu) < 300) {
    return false;
  } else {
    var expireTime = cGet('maskisu_e');
    if (expireTime == null) {
      expireTime = Date.parse(new Date()) + 60 * 1000;
      cSet('maskisu_e', expireTime);
    }
    popShow();
    cycleCountdown();
  }
...
function getRetime(isInit) {
  if (isInit == 2) {
    ...
    var reTime = 6.3;
    var packsNumTotal = cGet('endecisuxr_t');
    if (packsNumTotal > 1) {
