...
function soldOut() {
	...
	$(".banner .right-message .progress").html("<h3>IObit Software Updater 2 PRO</h3> <p>Roczna subskrypcja / 3 PC</p>")
  ...
  $(".banner .right-message ul span:eq(2)").html("70,00 zł")
	$(".floatlayer .number").html("<h2>Zaktualizuj swoje programy Napraw dziury w bezpieczeństwie</h2>");
	...
	$(".buybtn").attr({
		"href": 'http://www.iobit.com/buy.php?product=pl-isu3pc49&ref=pl_isu3pc49purchase1906'+refStr+'&refs=pl_purchase_isu&tw=' + tw,
		"onclick": "ga('send', 'event', 'isubuy', 'buy', 'isupurchase3PC1906-pl') "
	});
}
...
function setUnit(packsNum) {
	var unitStrBefore = 'pozostało tylko';
	var unitStrAfter = 'miejsc';
	var packsLast = packsNum % 10;
	var packsLastTwo = packsNum % 100;
	if (packsNum == 1) {
		unitStrBefore = 'pozostało ';
		unitStrAfter = 'miejsce';
	} else if (($.inArray(packsLast, [2, 3, 4]) != -1)&&($.inArray(packsLastTwo,[12,13,14])==-1)) {
		unitStrBefore = 'pozostały tylko';
		unitStrAfter = 'miejsca';
	}
