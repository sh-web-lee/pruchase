...
function getCycleCountdown() {
  var date = new Date();
  if (MApp(2.2).cookie.get('itimfxmas2019')) {
    dt = MApp(2.2).cookie.get('itimfxmas2019');

  } else {
    dt = date.setMinutes(date.getMinutes() + 10);
    MApp(2.2).cookie.set('itimfxmas2019', dt);
  }
...
function getPacksNum() {
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('itimfxmas2019') / 1000;
...
function soldOut() {
  ...
  //comparison
  $(".comparison h3 span").text("65%");
  $(".comparison .buybtn").attr({
    "href": $imf3pc.attr("href"),
    "onclick": "ga('send', 'event', 'imfbuy', 'buy', 'imfpurchase3pc1911mbxmas-it')"
  });
  //bottom-buy
