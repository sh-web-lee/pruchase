...
function changeStart() {
  ...
  cstar.set(".intro-list", {
    left: 219,
    top: -50,
  }).set($('.intro .computer.two,.intro .shield,.intro .safe,.intro .safe-words'), {
    opacity: 0,
...
function soldOut() {
  ...
  if ($(".compare-wrap").hasClass("new")) {
    $(".feature h2").html("Activate all features with <span>55% OFF</span> today!");
  }
  $(".change-price").html('$<big>21</big>.99');
  $(".popbox .change-price").html('$<big>16</big>.99');
  $(".buybtn.exbtn.soldout").attr({
    "href": "https://www.iobit.com/buy.php?product=imf73pc2199x&ref=xr_imf2199" + refStr + "&refs=purchase_imfx",
    "onclick": "ga('send', 'event', 'imfbuysold', 'buy', 'imfpurchase-xmas);",
  });
}
...
function mask(scroll) {
  ...
  maskTimeiu = cGet('maskimf_t');

  if ((scroll == 1) && (_maskTimetwoiu - maskTimeiu) < 300) {
    return false;
  } else {
    var expireTime = cGet('maskimf_e');
    if (expireTime == null) {
      expireTime = Date.parse(new Date()) + 60 * 1000;
      cSet('maskimf_e', expireTime);
    }
    popShow();
    cycleCountdown();
  }
...
function getRetime(isInit) {
  if (isInit == 2) {
    ...
    var reTime = 6.3;
    var packsNumTotal = cGet('endecimfxr_t');
    if (packsNumTotal > 1) {
