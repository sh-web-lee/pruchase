...
function getCycleCountdown() {
  var date = new Date();
  if (MApp(2.2).cookie.get('novimfxmasexpit2019')) {
    dt = MApp(2.2).cookie.get('novimfxmasexpit2019');

  } else {
    dt = date.setMinutes(date.getMinutes() + 10);
    MApp(2.2).cookie.set('novimfxmasexpit2019', dt);
  }
...
function getPacksNum() {
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('novimfxmasexpit2019') / 1000;
  var n2 = Math.floor(e.getTime() / 1000);
  if (n1 - n2 > 0) {
    packs = Math.floor((n1 - n2) / 22.5);
  } else packs = 0;
  $('.packsNum').html(packs);
  progressbar(packs);
...
(function() {
  ...
  $(window).on('scroll', function() {
    if ($(window).scrollTop() > $(".banner .buybtn").offset().top) {
      $('.intro h2,.intro dl,.intro .ball,').addClass("on");
    }
  });
  //bottom-buy
