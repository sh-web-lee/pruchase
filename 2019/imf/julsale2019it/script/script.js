...
function getMessage(_this) {
  ...
  $('.bottom-box img').stop().animate({ 'marginLeft': index * (-313) }, 300);
  $('.bottombuy .price b').text(message.price);
  $('.bottombuy .price del').text(message.del);
  $('.bottombuy .price i').text(message.off);
  $('.bottombuy .btn').attr('href', message.url).attr('onclick', message.ga);
}

function overoff() {
  $('#mainBundle').html('').addClass('over');
  $('#gift').addClass('over').html('<div class="bg"></div>');
...
$(function () {
  ...
  $('#sliderSwitch').on('mouseenter', '.text', function () {
...
var countdown = function (countdown, tType) {
  ...
  $('.countdown:eq(0) span').eq(0).html(i).end().eq(1).html(s).end().eq(2).html(mi);
  $('.countdown:eq(1) span').eq(0).html(i).end().eq(1).html(s).end().eq(2).html(mi);
  if (statusNow == 1) {
...
function getPacksWithGift() {
  ...
  var packsNow;
  packsNow = Math.ceil(timeDiff / 8.96) > 0 ? Math.ceil(timeDiff / 8.96) : 0;
  if (statusCheck() == 1) {
    if (packsNumGift != packsNow) {
      packsNumGift = packsNow;
      $('.packsNum').html(packsNumGift);
      $('.buyNum').html(500 - packsNumGift);
    }
    setTimeout('getPacksWithGift()', 10000);
  }
}