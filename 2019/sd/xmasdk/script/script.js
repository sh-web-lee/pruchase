...
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('xmassalesfr2019')) {
    dt = MApp(2.2).cookie.get('xmassalesfr2019');
  
  }else{
    dt=date.setMinutes(date.getMinutes()+8);
    MApp(2.2).cookie.set('xmassalesfr2019', dt);
  }
...
function sect02() {
  ...
  tl.to(".list02 .privacy-right", 1, {opacity:1,scale: 1});
}
