...
function prev() {
  $(".dg-prev").click(function() {
    ...
    $('.review .reviews p').removeClass("on");
    $('.dg-container ul li:eq(0)').removeClass("one");
    ...
    $('.dg-container ul li:eq(0)').addClass("one");
    ...
    setTimeout($('.review .reviews').eq(num).find("p").addClass("on"), 500);
  });
}
...
function getRetime(isInit) {
  if (isInit == 2) {
    ...
    var packsNumTotal = cGet('novxascdb2019it_t');
    if (packsNumTotal > 1) {
