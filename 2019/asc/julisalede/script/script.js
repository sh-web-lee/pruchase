...
function getSelect(selector) {
  ...
  var _url = $(".banner li").eq(_num).find(".buybtn").attr("href");
  var _ga = $(".banner li").eq(_num).find(".buybtn").attr("onclick");
  var _price = $(".banner li").eq(_num).find(".price strong").text();
  var _del = $(".banner li").eq(_num).find(".price del").text();
  selector.addClass("active").siblings().removeClass('active');
  $(".bottombuy .box img").stop().animate({
    "marginLeft": index * (-240)
  }, 300);
...
function packsInit() {
  var date = new Date();
  var cName = "deascoff_p" + date.getDate();
...
function timeInit() {
  var date = new Date();
  var cName = "dedbdecny_t" + date.getDate();
  var arr = document.cookie.match(new RegExp("(^| )" + cName + "=([^;]*)(;|$)"));
...
function statusSet() {
  ...
  if ((packs <= 0) && (timeNow > countTime) && (popShow == 0)) {
    var cNameMask = "dedboffta_m" + date.getDate();
...
function mouseOut(e) {
  if (e.clientY < 3) {
    showPop();
    var date = new Date();
    var cName = "dedboffta_m" + date.getDate();
    cSet(cName, 1);
...
//add pop
function showPop() {
  $(".pop").removeClass("mini").fadeIn(100, function() {
    popShow = 1
  });
