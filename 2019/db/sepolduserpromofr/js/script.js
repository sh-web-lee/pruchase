...
//
var string = "Jusqu’à -85% sur Driver Booster PRO et obtenez un abonnement de 2 mois supplémentaires gratuitement, viiite !";
...
function afterThirtySeconds () {
  ...
  $("#db-3pc p.attention").addClass("hidden");
  $("#db-3pc p.countdown").addClass("hidden");
...
$(function () {
  ...
  $(".bottomcart ul li").click(function () {
    ...
    if(num !== 2) {
      $(this).addClass("active").siblings().removeClass("active");
      $(".bottomcart .buybtn").attr("href",$(this).attr("data-url")).attr("onclick",$(this).attr("data-ga"));
      $(".bottomcart p.price strong").text($(this).attr("data-price"));
      $(".bottomcart p.price del").text($(this).attr("data-original"));
      if(num === 0) {
        $(".bottomcart a.buybtn").addClass("cart").html("<span>VITE, J’EN PROFITE</span>");
        $(".bottomcart .img").removeClass("three-pc").removeClass("one-pc");
        $(".bottomcart p.price").removeClass("after");
      }else if(num === 1) {
        $(".bottomcart a.buybtn").removeClass("cart").html("<span>Activez Maintenant</span>");
        $(".bottomcart .img").addClass("three-pc").removeClass("one-pc");
        $(".bottomcart p.price").addClass("after");
      }
    }
  });
  timeInit();
