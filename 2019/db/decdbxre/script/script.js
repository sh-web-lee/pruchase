...
function soldOut() {
  ...
  $(".change").html('Sorry, 80% discount is sold out.');
  $(".change-title").html("<p>Oops, 80% discount SOLD OUT!</p><h2>However, <span>75% discount</span> is still available.</h2>");
  $(".change-price").html('$<big>16</big>.99');
  $(".popbox .change-price").html('$<big>12</big>.85');
  $(".exbtn.soldout").attr({
    "href": 'https://www.iobit.com/buy.php?product=2019db3pc1699&ref=xr_dbe1699' + refStr + '&refs=purchase_dbx',
    "onclick": "ga('send', 'event', 'dbebuysold', 'buy', 'dbepurchase-xmas');",
  });
}
...
function mask(scroll) {
  ...
  if ((scroll == 1) && (_maskTimetwoiu - maskTimeiu) < 600) {
    ...
    var expireTime = cGet('maskdbxre_e');
    if (expireTime == null) {
      expireTime = Date.parse(new Date()) + 60 * 1000;
      cSet('maskdbxre_e', expireTime);
    }
    popShow();
    cycleCountdown();
  }
...
function getRetime(isInit) {
  if (isInit == 2) {
    ...
    var reTime = 6.3;
    var packsNumTotal = cGet('endbdecdbxre_t');
    if (packsNumTotal > 1) {
