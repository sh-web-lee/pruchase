...
function addUrl(name) {
	switch (name) {
		case "sd":
			$(".buybtn").attr({
				"href": "https://www.iobit.com/buy.php?product=db6sd&ref=db6sdnewtkasec" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
				"onclick": "ga('send', 'event', 'db6sdnewbuy', 'buy', 'dbsdpurchase-tkasec')",
			});
			break;
		case "iu":
			$(".buybtn").attr({
				"href": "https://www.iobit.com/buy.php?product=db6iu&ref=db6iunewtkasec" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
				"onclick": "ga('send', 'event', 'db6iunewbuy', 'buy', 'dbiupurchase-tkasec')",
			});
			break;
		case "iusd":
			$(".buybtn").attr({
				"href": "https://www.iobit.com/buy.php?product=db6sdiu&ref=db6sdiunewtkasec" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
				"onclick": "ga('send', 'event', 'db6sdiunewbuy', 'buy', 'dbsdiupurchase-tkasec')",
			});
			break;
		default:
	}
...
function soldProduct() {
	...
	$(".buybtn").attr({
		"href": "https://www.iobit.com/buy.php?product=driverbooster62395&ref=db6soldnewtkasec" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
		"onclick": "ga('send', 'event', 'db6soldnewbuy', 'buy', 'dbsoldpurchase-tkasec')",
	});
	$(".subscript em").text("3");
}
// soldProduct();
		...
		if (!$(this).hasClass("on")) {
			...
			if (0 < length < 4) {
				...
				if (arrone === 1) {
					...
					$(".banner .price").append('<p class="only">only $1.99 / Month </p>');
					$(".buybtn").attr({
						"href": "https://www.iobit.com/buy.php?product=driverbooster62395&ref=db6newtkasec" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
						"onclick": "ga('send', 'event', 'db6newbuy', 'buy', 'dbpurchase-tkasec')",
					});
					$(".banner .price .tips").remove();
				}
			}
...
function timeInit() {
	var date = new Date();
	var cName = "endbanni_tnew" + date.getDate();
