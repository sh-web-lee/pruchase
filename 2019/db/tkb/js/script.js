...
function soldout() {
	...
	$(".floatlayer ul li.packs").html("<p>3 Months</p><strong>$<b>4.33</b></strong>/mo<p class='last'>Save 40%</p>")
	$(".floatlayer .buybtn,.banner .box:eq(1) .buybtn,.comparison .itemd .buybtn").attr("href", "https://www.iobit.com/buy.php?product=db3m&ref=db3msold"+refStr+"&aff=" + aff + "&refs=purchase_db&tw=" + tw);
  $(".floatlayer .buybtn,.banner .box:eq(1) .buybtn,.comparison .itemd .buybtn").attr("onclick", " ga('send', 'event', 'db3msoldbuy', 'buy', 'dbpurchase-tkb')");
  $(".banner .box:eq(0) .buybtn,.comparison .itemc .buybtn").attr("href", "https://www.iobit.com/buy.php?product=db1m&ref=db1msold"+refStr+"&aff=" + aff + "&refs=purchase_db &tw=" + tw);
  $(".banner .box:eq(0) .buybtn,.comparison .itemc .buybtn").attr("onclick", "ga('send', 'event', 'db1msoldbuy', 'buy', 'dbpurchase-tkb')");
...
$(".floatlayer ul li").click( function () {
  ...
  if(!$(".banner .packs").hasClass("sold-out")) {
    if(index==0) {
    	...
    	$(".floatlayer .buybtn").attr("href", $(".banner .box").eq(2).find(".buybtn").attr("href"));
    	$(".floatlayer .buybtn").attr("onclick", $(".banner .box").eq(2).find(".buybtn").attr("onclick"));    
    }
  }
  else {
...
function timeInit() {
	var date = new Date();
	var cName = "endbtkb_t" + date.getDate();
