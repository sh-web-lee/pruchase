...
var chartShow = function () {
  ...
  var chartSet = setInterval(function () {
    if (i<$(".chart dl").length) {
      $(".chart dl dd:nth-child("+ i +")").addClass("on");
      i++;
    } else {
      $(".right-message h2, .right-message h3").addClass("on");
...
function setDateStr(daysDiff,dateIndex){
  ...
  $(".chart dl").eq(dateIndex).find(".text span").html(dateSrt);
}
...
function getReTime(){
  var reTime=20;
  var packsNumTotal=cGet('itdbsum_t');
  console.log(packsNumTotal);
  console.log(packs);
  if(packsNumTotal>1) {
    if(packs>32){
