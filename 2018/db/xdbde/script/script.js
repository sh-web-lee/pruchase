...
function maskFirst() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Überraschung zu Weihnachten</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Driver Booster 6 PRO"/></div><div class="price fl"><h3><b>1€ Rabatt</b> nur für Sie!</h3><p><strong>18</strong><span><del>159,93€</del><b>,99€</b></span></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Jetzt aktivieren</a><p class="year">Letzte Chance auf den größten Rabatt des Jahres!</p></div></div></div>');
}
...
function timeInit() {
  var date = new Date();
  var cName = "dedbpre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "dedb" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 4.11) > 0 ? Math.ceil(timeDiff / 4.11) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 4.11) > 0 ? Math.ceil(timeDiff / 4.11) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
