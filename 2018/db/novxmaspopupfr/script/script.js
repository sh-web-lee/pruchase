...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Surprise de Noël </h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Driver Booster PRO"/></div><div class="price fl"><h3><b>1€</b> de réduction exclusivement pour vous !</h3><p><strong>18<big>, </big></strong><span>99€ <del>174,82€</del></span></p><a class="buybtn" href="' + _href + '" onclick="' + _ga + '">VITE, J’EN PROFITE</a><p class="year">Saisissez votre dernière chance</p></div></div></div>');
...
function packsInit() {
  var date = new Date();
  var cName = "frdbpre" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 5.18) > 0 ? Math.ceil(timeDiff / 5.18) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 5.18) > 0 ? Math.ceil(timeDiff / 5.18) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
