...
function soldOut() {
  ...
  $(".centbuy dl.amc").find("img").attr("src", amcimgSrc);
  $(".centbuy").find(".buybtn").attr("href", "javascript: void(0);");
  $(".footbuy img").eq("2").addClass("active").siblings().removeClass("active");
  $(".footbuy li").eq("2").addClass("active").siblings().removeClass("active");
}
...
function timeInit(){
  var date = new Date();
  var cName = "itdbnataleb_t" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "itdbnataleb_p" + date.getDate();
  var arr = document.cookie.match(new RegExp("(^| )" + cName + "=([^;]*)(;|$)"));
  if (arr == null) {
    packs = 49;
  } else {
