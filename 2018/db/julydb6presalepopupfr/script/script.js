$(window).on('scroll', function() {
	...
	let scrollHeight_box = $(".feature-db .update").offset().top;
	let scrollHeight_float = $(".comparision table .buybtn").offset().top;
	if ($(window).scrollTop() > scrollHeight_dl) {
		$('.container dl').addClass('on');
	}
	if ($(window).scrollTop() > scrollHeight_box) {
		$('.sf-box').addClass('on');
...
function packsInit() {
	var date = new Date();
	var cName = "endbprepop" + date.getDate();
