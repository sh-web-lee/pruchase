...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Julöverraskning</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Driver Booster PRO 6"/></div><div class="price fl"><h3>¡Extra <b> 1€</b> som <br> de descuento sólo para ti!</h3><p><strong>13<big>,99€</big></strong> <del>99,93€ </del></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Comprar ahora</a><p class="year">¡Aprovecha tu última oportunidad para ahorrar más!</p></div></div></div>');
...
function timeInit() {
  var date = new Date();
  var cName = "esiupre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "esiu" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 5.8) > 0 ? Math.ceil(timeDiff / 5.8) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 5.8) > 0 ? Math.ceil(timeDiff / 5.8) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
