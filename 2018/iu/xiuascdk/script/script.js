...
function timeInit() {
  var date = new Date();
  var cName = "dkascpre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "dkiuasc" + date.getDate();
  var arr = document.cookie.match(new RegExp("(^| )" + cName + "=([^;]*)(;|$)"));
  if (arr == null) {
    packs = 70;
  } else {
...
function cycleCountdown(){
  ...
  var datetime = MApp(2.2).datetime.getCycleCountdown(startTimestamp, 2);
  ...
  $('.countdown').each(function() {
    $(this).find('b').eq(0).html(d).end().eq(1).html(h).end().eq(2).html(i).end().eq(3).html(s);
  });
