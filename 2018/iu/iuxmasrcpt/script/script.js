...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Feliz Natal </h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="IObit Uninstaller 8 PRO"/></div><div class="price fl"><h3> <b>R$ 3 </b>  extra de desconto</h3><p> Apenas<strong> <small> R$ </small>48<small>,99</small> </strong>  1 ano / 3 PCs</p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Comprar Agora</a><p class="year">Sua última chance de aproveitar <br>  o preço mais baixo da história! </p></div></div></div>');
...
function timeInit() {
  var date = new Date();
  var cName = "ptiupre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "ptiu" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 4.15) > 0 ? Math.ceil(timeDiff / 4.15) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 4.15) > 0 ? Math.ceil(timeDiff / 4.15) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
