...
function maskFirst() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Überraschung zu Weihnachten</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt=""/></div><div class="price fl"><h3><b>1€</b> Rabatt nur für Sie</h3><p><strong>15<big>,99€</big></strong> <del>109,89€</del></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Jetzt aktivieren</a><p class="year">Letzte Chance auf den größten Rabatt des Jahres!</p></div></div></div>');
}
...
function timeInit() {
  var date = new Date();
  var cName = "deiupre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "deiu" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 3.9) > 0 ? Math.ceil(timeDiff / 3.9) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 3.9) > 0 ? Math.ceil(timeDiff / 3.9) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
