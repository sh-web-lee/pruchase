...
function reduceNum(num, numRe) {
  ...
  setTimeout('$(".progressbar .reduce").addClass("zoomout");', 200);
...
function maskFirst() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Surprise de Nouvel An</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Advanced SystemCare 12 Pro"/></div><div class="price fl"><h3>Rabais de <b>1 €</b> pour vous exclusivement !</h3><p><strong>18</strong><span>99€ <del>79,98€</del></span></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">VITE, J’EN PROFITE</a><p class="year">Saisissez votre dernière chance</p></div></div></div>');
}
...
function packsInit() {
  ...
  packs = Math.ceil(timeDiff / 5.3) > 0 ? Math.ceil(timeDiff / 5.3) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 5.3) > 0 ? Math.ceil(timeDiff / 5.3) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
