...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Kerstcadeautje</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Advanced SystemCare 12 Pro"/></div><div class="price fl"><h3> <b>€1</b>  extra KORTING speciaal voor jou!</h3><p>  <strong> <big> Slechts €</big>18<big></big></strong> <span><del>€139,93</del>99</span></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Activeer Nu</a><p class="year">Grijp uw laatste kans!</p></div></div></div>');
...
function timeInit() {
  var date = new Date();
  var cName = "nlimfpre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "nlimf" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 4.9) > 0 ? Math.ceil(timeDiff / 4.9) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 4.9) > 0 ? Math.ceil(timeDiff / 4.9) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
