...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>COMPLIMENTI!</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Driver Booster 6 PRO"/></div><div class="price fl"><h3>Hai vinto <b>€1</b>  di extra sconto esclusivo! </h3><p><i>SOLO</i> <strong>18<big>,</big></strong> <span>99 <del>€149,78</del></span></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Approfittane Ora</a><p class="year"> Cogli questa ultima opportunità per il <br> grande risparmio in assoluto! </p></div></div></div>');
...
function timeInit() {
  var date = new Date();
  var cName = "itimfpre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "itimf" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 6.3) > 0 ? Math.ceil(timeDiff / 6.3) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 6.3) > 0 ? Math.ceil(timeDiff / 6.3) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
