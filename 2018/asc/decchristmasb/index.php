...
if($_GET['action']=='ipCheck'){
	...
	$ipInfo = @file_get_contents("http://ip.taobao.com/service/getIpInfo.php?ip=".$ip, false, $context);
	$ipArr = json_decode($ipInfo);
	$countryName=$ipArr->data->country;
	if($countryName=='土耳其'){
		trLog();
		echo 1;
	}else{
...
function trLog(){
	try{
		...
		$insertSql="insert into trlog(ip,lan,addTime,page) values(:ip,:lan,:addTime,0)";
...
<title>Save up to 85% OFF. New Year Exclusive Discount - IObit</title>
...
<?php echo $gJavascript['public'];?>
	<script>
		...
		if(browserLanguage=='tr'){
			...
			var deviceStr = MApp(2.2).url.getParameters('st');
			var cName='';
			var pageAb='';
			var randNum=0;
			if((deviceStr=='asc_promotec')||(deviceStr=='asc_promoted')){
				window.location.replace('./privacy.php' + queryStr);
			}else{
				cName = 'enascptestab';
				pageAb = cGet(cName);
				if (pageAb == 'tob') {
					window.location.replace('./anniversary.php' + queryStr);
				} else {
					if (pageAb == null) {
						randNum = Math.random();
						if (randNum > 0.5) {
				...
				<div class="defult">
					...
					<div class="price-box">
						<div class="price-btn clearfix">
							...
							<a href="https://www.iobit.com/buy.php?product=asc12sdiuamc2198&ref=ascsdiuamc2198may2017b<?php echo $refStr;?>&aff=<?php echo $_GET['aff'];?>&refs=purchase_asc" onclick="ga('send', 'event', 'asc2198buy', 'buy', 'ascpurchase-201812');clickCount(2)" class="buy-btn banner-btn-words fr buy-link"><i class="car"></i><span>ACTIVATE NOW</span></a>
						</div>
						<ul class="price-dec">
							<li class="first">
								<span>Original</span>
	...
	<div class="hight-light clearfix">
		...
		<div class="original">
			<dl class="list1 animate fl">
				...
				<dd>
					<h3>200%*</h3>
					<p>Reduce system stutter/lag and speed upPC up to 200% faster</p>
				</dd>
			</dl>
			...
			<dl class="list3 animate fl">
				...
				<dd>
					<h3>Deeper</h3>
					<p>Fix registry issues to reduce Windows crashes & error messages</p>
				</dd>
			</dl>
			<dl class="list4 animate fr">
				...
				<dd>
					<h3>Auto</h3>
					<p>Auto stop unused processes to release RAM and improve PC performance</p>
				</dd>
			</dl>
			<dl class="list5 animate fl">
				...
				<dd>
					...
					<p>
						Keep your sensitive data like contact, e-mail data, Dropbox, OneDrive etc. away from any secret access.
				  </p>
				</dd>
			</dl>
			<dl class="list6 animate fr">
				...
				<dd>
					...
					<p>Auto clear browsing data and disguise digital fingerprint to stop malicious tracking.</p>
				</dd>
			</dl>
		</div>
		<?php else:?>
		<div class="winrar">
			<dl class="list1 animate fl">
				...
				<dd>
					<h3>Faster</h3>
					<p>Reduce system stutter for up to <b>200%*</b> faster startup and accelerate internet connection for <b>300%*</b> faster browsing</p>
				</dd>
			</dl>
			<dl class="list2 animate fr">
				<dt><img src="<?php echo $pResUrl; ?>images/hightlight-icon_win.png" alt=""></dt>
				<dd>
					<h3>Safe Software Update <i>ATTENTION</i></h3>
					<p>
						1-click to update important software for you, including <b>WinRAR*, whose existing security flaw may put your computer at risk for years.</b>
						<span>*Update immediately, once the new official version is ready.</span>
					</p>
				</dd>
			</dl>
			<dl class="list3 animate fl">
			...
			<dl class="list4 animate fr">
				...
				<dd>
					<h3>Auto</h3>
					<p>Auto stop unused processes to release <br>RAM and improve PC performance</p>
				</dd>
			</dl>
			<dl class="list5 animate fl">
				<dt><img src="<?php echo $pResUrl; ?>images/hightlight-icon_13.png" alt=""></dt>
...
<div class="middle-buy container clearfix">
	<div class="wrapper">
		<div class="title">
			<h2>Activate Advanced SystemCare PRO Today</h2>
			<p>Get 3 PC Optimization Tools (<span>valued at $59.97</span>) for FREE!</p>
		</div>
		...
		<div class="btn-content">
			<a href="https://www.iobit.com/buy.php?product=asc12sdiuamc2198&ref=ascsdiuamc2198may2017b<?php echo $refStr;?>&aff=<?php echo $_GET['aff'];?>&refs=purchase_asc" onclick="ga('send', 'event', 'asc2198buy', 'buy', 'ascpurchase-201812');clickCount(2)" class="buy-btn middle-btn">
					<i class="car"></i>
		...
		<div class="bar-banner">
			...
			<div class="bar-right fr">
				...
				<div class="price-btn bar-btn">
					<span class="price-num">Now $<strong>21</strong>.98</span>
					<a href="https://www.iobit.com/buy.php?product=asc12sdiuamc2198&ref=ascsdiuamc2198may2017b<?php echo $refStr;?>&aff=<?php echo $_GET['aff'];?>&refs=purchase_asc" onclick="ga('send', 'event', 'asc2198buy', 'buy', 'ascpurchase-201812');clickCount(2)" class="buy-btn banner-btn-words buy-link"><i class="car"></i><span>ACTIVATE NOW</span></a>
				</div>
			</div>
		</div>
	</div>
		...
		<tfoot>
			<tr>
				...
				<th class="item-pro">
					...
					<a href="https://www.iobit.com/buy.php?product=asc12sdiuamc2198&ref=ascsdiuamc2198may2017b<?php echo $refStr;?>&aff=<?php echo $_GET['aff'];?>&refs=purchase_asc" onclick="ga('send', 'event', 'asc2198buy', 'buy', 'ascpurchase-201812');clickCount(2)" class="buy-btn smail-btn"><i class="car"></i><span>ACTIVATE NOW</span></a>
					<p class="table-peice">85% OFF Save $127.96</p>
				</th>
			</tr>
		</tfoot>
	</table>
...
<!-- mask -->
<div class="mask container" style="display: none;"></div>
<div class="mask-box" style="display: none;">
	<h2>You are today's Lucky Visitor!<br>
		Enjoy Your <b>Exclusive Extra 10% OFF </b>Now!
		<i class="close">×</i>
	</h2>
	<div class="mask-main clearfix">
		<img class="small-img" src="<?php echo $pResUrl; ?>images/asc-small.png">
		<div class="mask-main-right fl">
			...
			<ul class="price-dec mask-list clearfix">
				...
				<li>
					<span class="font-gray">After extra 10% off</span>
					<p>$<b>19</b>.99</p>
				</li>
			</ul>
			<div class="btn-box">
				<a href="https://www.iobit.com/buy.php?product=asc12sdiuamc1999&ref=ascsdiuamc1999may2017b<?php echo $refStr;?>&aff=<?php echo $_GET['aff'];?>&refs=purchase_asc" onclick="ga('send', 'event', 'asc1999buy', 'buy', 'ascpurchase-201812');clickCount(3)" class="buy-btn mask-btn "><i class="car"></i><span>Get My Extra 10% OFF &amp; 3 FREE Gifts</span></a>
			</div>
