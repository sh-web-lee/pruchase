function soldOut() {
  ...
  $(".banner .sold-box dl dt strong").html("58%");
  $(".banner .one-pc-box dl dt strong").html("85%");
  $(".banner .one-pc-box dl .button").html("Approfittane Ora!");
  $(".banner .sold-box h3").html("Bara <span class='num'><span class='pack packsNum'>25</span> <span class='popup-font packsNum'>25</span></span> paket kvar");
  $(".selection li").eq(0).addClass("default").removeClass("active");
  $(".selection li").eq(1).addClass("active");
  $(".gifts dl").addClass("exceed");
  $(".banner .sold-box .button,.gifts .button").attr("href", "https://www.iobit.com/buy.php?product=sv-asc123pc249&ref=sv_asc123pc249purchase1811"+refStr+"&refs=sv_purchase_asc");
  $(".banner .sold-box .button,.gifts .button").attr("onclick", "ga('send', 'event', 'ascbuy', 'buy', 'ascpurchase3PC1811-sv')");
  onePc();
}
...
function onePc() {
  $(".bottomcart .button").attr("href", "https://www.iobit.com/buy.php?product=sv-asc123pc249&ref=sv_asc123pc249purchase1811"+refStr+"&refs=sv_purchase_asc");
  $(".bottomcart .button").attr("onclick", "ga('send', 'event', 'ascbuy', 'buy', 'ascpurchase3PC1811-sv')");
  $(".bottomcart dl strong").html("<b>249</b>KR");
  $(".bottomcart dl dt del").html("599KR");
  $(".bottomcart dl dt span").html("350kr");
  $(".showcase .img").addClass("one-pc");
}

function threePc() {
  $(".bottomcart .button").attr("href", "https://www.iobit.com/buy.php?product=sv-asc123pcsdpf249x&ref=sv_asc123pcsdpfpurchase1811"+refStr+"&refs=sv_purchase_asc");
  $(".bottomcart .button").attr("onclick", "ga('send', 'event', 'ascbuy', 'buy', 'ascpurchase3PCsdpf1811-sv')");
  $(".bottomcart dl strong").html("<b>249</b>KR");
  $(".bottomcart dl del").html("1 557KR");
  $(".bottomcart dl dt span").html("1 308kr");
...
function timeInit() {
  var date = new Date();
  var cName = "svascjan" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "svascjanpa" + date.getDate();
  var arr = document.cookie.match(new RegExp("(^| )" + cName + "=([^;]*)(;|$)"));
  if (arr == null) {
    packs = 42;
  } else {
