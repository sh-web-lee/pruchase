...
function timeInit() {
  var date = new Date();
  var cName = "ascpsdpre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "ascpsdprepa" + date.getDate();
...
$(function() {
  ...
  $(window).scrollWindow($(".together dl.four"), $('.floatlayer'), ("on"));
  $(window).scrollWindow($(".banner .buybtn"), $('.feature-icons dl'), ("on"));
  $(window).scrollWindow($(".banner .buybtn"), $('.feature-message'), ("on"));
  $("#backtotop").target($(".banner"), 0);
  numberJump($('.viewNum'), viewNum, 500, 1500);
  $(window).on('scroll', function() {
    if ($(window).scrollTop() > $(".feature dl.four").offset().top) {
