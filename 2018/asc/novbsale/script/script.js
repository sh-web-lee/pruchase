...
function soldOut() {
    TweenMax.to($('.cut-box'), 3, {
        ...
        onComplete: function() {
            ...
            $(".buy-btn").attr("href", "https://www.iobit.com/buy.php?product=asc123pc2198&ref=asc3pc2198may2017" + refStr + "&aff=" + aff + "&refs=purchase_asc&tw=" + tw).attr('onclick', "ga('send', 'event', 'asc3pcbuy', 'buy', 'ascpurchase-201705');clickCount(4)");
            $("#floatlayer,.gotop").hide();
        }
    });
}
function change() {
    var packs = '<h2>No More Free Gifts,<br> Seize Your Last Chance To Get <strong>75% OFF</strong> Now!</h2>'+
    '<h4 class="title-h4">Advanced SystemCare 12 PRO</h4>'+
    '<h3>Limited quantity available</h3>'

    var details = '<li class="first"><span>Original</span><p>$89.97 </p></li><li><span>Discount</span><p class="font-green">75% OFF</p></li><li><span>Save</span><p>$67.99</p></li>'
    
    var between ='<h2>Seize Your Last Chance To Get 75% OFF</h2>'+
        ...
        '<div class="bar-right fr">'+
            ...
            '<a class="buy-btn"><i></i><span>ACTIVATE NOW</span></a>'+
            '</div>'+
        '</div>'
    $('.defult .title').html(packs);
    $('.defult .price-dec').html(details);
    $(".middle-buy .wrapper").html(between);
    $(".defult,.middle-buy").addClass("sold-out");
    $(".progress").remove();
}
