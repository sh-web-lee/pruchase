...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Sorpresa di Natale</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Advanced SystemCare 12 Pro"/></div><div class="price"><h3>Cogli l’ultima occasione per il grande risparmio con 60% di sconto!</h3><p><strong>21<big>,</big></strong> <span>99 <del>€49,99</del></span></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Approfittane Ora!</a></div></div></div>');
...
function packsInit() {
  ...
  packs = Math.round(timeDiff / 5.6) > 0 ? Math.round(timeDiff / 5.6) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.round(timeDiff / 5.6) > 0 ? Math.round(timeDiff / 5.6) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
