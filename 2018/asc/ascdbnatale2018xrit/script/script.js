...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Sorpresa di Natale</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Advanced SystemCare 12 Pro"/></div><div class="price"><h3>Hai vinto <b>1€</b> di extra sconto esclusivo!</h3><p> <i>Solo</i> &nbsp;&nbsp; <strong>28<big>,</big></strong> <span>99 <del>€124,84</del></span></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">APPROFITTANE ORA!</a><p class="year">Cogli questa ultima opportunità per il grande <br> risparmio in assoluto!</p></div></div></div>');
...
function packsInit() {
  var date = new Date();
  var cName = "frasc" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 5.77) > 0 ? Math.ceil(timeDiff / 5.77) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 5.77) > 0 ? Math.ceil(timeDiff / 5.77) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
