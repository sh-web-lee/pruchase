...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Offerta di Natale </h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Advanced SystemCare 12 Pro"/></div><div class="price"><h3> Grijp Uw Laatste Kans Om Dit <br> Jaar een Hoop Geld te Besparen! </h3><p><strong><small>€</small>19<small>,</small></strong> <span>99<del>€49,99</del></span></p><a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Activeer Nu</a><p class="year"> 1-jarig abonnement / 3 PC’s</p></div></div></div>');
...
function timeInit() {
  var date = new Date();
  var cName = "nlascsdpre" + date.getDate();
...
function packsInit() {
  ...
  packs = Math.ceil(timeDiff / 4.5) > 0 ? Math.ceil(timeDiff / 4.5) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 4.5) > 0 ? Math.ceil(timeDiff / 4.5) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
