...
function flipNum(num, length) {
  ...
  $(".btmcart .packbox .list").html(list);
  for (var a = 0; a <= len; a++) {
    ...
    $(".btmcart .packbox .list").eq(a).animate({
      marginTop: -curNum * 33,
    }, "slow");
  }
}
...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Offerta di Natale </h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Advanced SystemCare 12 Pro"/></div><div class="price"><h3>Cogli l’ultima occasione per il <br> grande risparmio con <b>60%</b> di sconto! </h3><p><strong>19<small>,</small></strong> <span>99<del>€49,99</del></span></p><a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Approfittane Ora!</a><p class="year"> Abbonamento 1 anno / 3 PC</p></div></div></div>');
...
function timeInit() {
  var date = new Date();
  var cName = "ascsdpre" + date.getDate();
...
function packsInit() {
  var date = new Date();
  var cName = "sdasc" + date.getDate();

  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  packs = Math.ceil(timeDiff / 6.4) > 0 ? Math.ceil(timeDiff / 6.4) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 6.4) > 0 ? Math.ceil(timeDiff / 6.4) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
...
function getViewNum() {
  $.ajax({
    type: "GET",
    url: "xsdpasc.php",
    data: "action=getViewNum&p=" + packs,
