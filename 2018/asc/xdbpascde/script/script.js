...
function maskShow() {
  ...
  $('.main').append('<div class="mask"></div><div class="mask-box"><h2>Die letzte Chance in diesem Jahr, um so richtig zu sparen!</h2><div class="clearfix"><div class="fl left"><img src="' + imgSrc + '" alt="Advanced SystemCare 12 Pro"/></div><div class="price"><h3>60% RABATT</h3><p><strong>23<big>,</big></strong> <span>99€ <del>59,99€</del></span></p> <a class="buybtn" href="' + _href + '" onclick="' + _ga + '">Jetzt aktivieren</a><p class="year">1Jahr/ 3 PC´s</p></div></div></div>');
...
function packsInit() {
  ...
  packs = Math.ceil(timeDiff / 3.75) > 0 ? Math.ceil(timeDiff / 3.75) : 0;
  reduceNum(packs, 1);
...
function getSales() {
  ...
  var nowTime = Math.floor(date.getTime() / 1000);
  var timeDiff = countTime - nowTime;
  var packsNow = Math.ceil(timeDiff / 3.75) > 0 ? Math.ceil(timeDiff / 3.75) : 0;
  if (packsNow != packs) {
    if (statusCheck() == 2) {
      packs = 0;
    } else {
