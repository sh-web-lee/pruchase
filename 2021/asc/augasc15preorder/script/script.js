...
function soldOut() {
  ...
  $(".price strong").after("<del>$89.97</del>")
  $(".banner .price del").after("<div class='pc'>1 Year Subscription / 3 PCs </div>")
  $(".banner h1").html(soldout_banner)
  $(".banner .numbox").remove();
  $(".floatlayer .numbox").html(soldout_float)
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=asc15preorder1677&ref=en-asc15preordersoldout'+ refStr +'&aff=' + aff + '&refs=purchase_asc&tw=-8',
    "onclick": "ga('send', 'event', 'ascsoldbuy', 'buy', 'asc15preorder');"
  });
}
...
function getSales(isInit) {
  ...
  $.ajax({
    type: "GET",
    url: "asc15preorder.php",
    data: "action=getPacks&d=" + date.getDate(),
    success: function(res) {
      var sales = JSON.parse(res);
      if (sales.packsNum != packs) {
        packs = sales.packsNum;
        $(".progress span").css("width",packs*2);
        if(isInit){
...
function getReTime() {
  var packsNumTotal = cGet('enascoff_t');
  if (packsNumTotal > 1) {
