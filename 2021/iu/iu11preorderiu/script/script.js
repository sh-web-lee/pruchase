...
function soldOut() {
  ...
  $(".sec1 i").css({ display: "none" })
  $(".sec1 .sec1-param, .sec1 .off").css({ display: "inline-block" });
  $(".price").find("strong").html("$14.77 <sub>$29.99</sub>");
  $(".sec3 .buybtn b").css({ display: "none" });
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=iu11preorder1477&ref=en-iu11preorderiusoldout' + refStr + '&aff=' + aff + '&refs=purchase_iu&tw=' + tw,
    "onclick": "ga('send', 'event', 'iusoldbuy', 'buy', 'iu11preorderiu');",
  });
}
// soldOut();
...
$(function() {
	...
	$(".pageations").on("click", ".pageation", function() {
		...
		$(".sec6 .text .item").eq(_index).addClass("on active").siblings().removeClass("on active");
	})
})
