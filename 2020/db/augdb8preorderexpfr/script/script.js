...
function soldout() {
    ...
    $(".right ._title_p").text('LA PRÉVENTE MONDIALE EST LÀ')
    $(".floatlayer .numbox").html('Le n°1 des utilitaires de driver updater à <strong> -65%</strong>');
    $(".buybtn").attr({
        "href": 'https://www.iobit.com/buy.php?product=fr-db8preorder2499&ref=fr_db8preorderexp' + refStr + '&aff=' + aff + '&refs=fr_purchase_db&tw=-8',
        "onclick": "ga('send', 'event',  'dbbuy', 'buy', 'db8preorderexp2499-fr');"
    })
    $(".price .buybtn").text("Renouveler Maintenant")
}
...
function getReTime() {
    var packsNumTotal = cGet('frdbexp_t');
    if (packsNumTotal > 1) {
        if ((packsNumTotal - packs) <= 30) {
            ...
            if (packsNumTotal <= 98 && packsNumTotal >= 90) {
                ...
                retime = (Math.random() > 0.5) ? 2.5 : 4.5;
            } else if (packsNumTotal <= 79 && packsNumTotal >= 70) {
                retime = (Math.random() > 0.5) ? 3 : 5;
            } else {
