...
function changeFreture(driveNum, gameNum) {
	...
	if (_num === 1) {
		...
		if ($(".feature").hasClass("update")) {
			$(".feature .content").prepend('' +
				'<dl class="five">' +
				'<dt class="driveNum">' + _driveNum + '</dt>' +
				'<dd>' +
				'<h3>drivrutiner är tillgängliga för uppdatering <sup>PRO</sup></h3>' +
				'</dd>' +
				'</dl>' +
				'<dl class="six">' +
				'<dt class="gameNum">' + _gameNum + '</dt>' +
				'<dd>' +
        '<h3>uppdatering av spelkomponenter tillgängliga <sup>PRO</sup></h3>' +
        '</dd>' +
        '</dl>');
		}
	}
...
function timeSet(){
	...
	$(".curvebox .four").html("Efter "+afterDaysArr[0][1]+' '+afterDaysArr[0][0]);
	$(".curvebox .one").html("Innan "+before3DaysArr[0][1]+' '+before3DaysArr[0][0]);
	if(leftDays==0){
		$(".days").html(1);
		$(".title p:eq(0)").html("Din rättegång slutar om mindre än 24 timmar.");
		$(".comparison tbody tr:eq(1),.comparison tbody tr:eq(2),.comparison tbody tr:eq(3),.comparison tbody tr:eq(4)").find(".itema").html("Förfaller om mindre än 24 timmar");
	}else{
		$(".days").html(leftDays);
	}
}
...
function getDateStr(daysArr,isBef){
	...
	if(tempArr.length==1){
		...
		if(isBef){
			return daysArr[0][1] + ' - ' + daysArr[1][0] + ' ' + daysArr[1][1] + ' ' + daysArr[0][0];
		}else {
			return daysArr[0][1] + ' - ' + daysArr[3][0] + ' ' + daysArr[3][1] + ' ' + daysArr[0][0];
		}
	}
