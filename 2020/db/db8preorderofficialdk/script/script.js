...
function soldout() {
  ...
  $(".price strong").remove();
  $(".banner .offer h2").html("Nr.1 mest downloaded opdateringsværktøj til drev");
  $(".price span").html("<strong><b>199</b> kr.</strong><del> 679 kr.</del>");
  $(".floatlayer .numbox").html('Få det mest downloaded opdateringsværktøj<br> med <strong>70% rabat</strong>');
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=dk-db8preorder199&ref=dk_db8preorderofficialsoldout' + refStr + '&aff=' + aff + '&refs=dk_purchase_db&tw=-8',
    "onclick": "ga('send', 'event', 'dbbuy', 'buy', 'db8preorderofficialsoldout-dk');"
  });
  $(".banner .buybtn").html("Forudbestil nu – 70% RABAT");
}
// soldout();
...
$(document).ready(function() {
  ...
  $('.review-box  span.prev').click(function(event) {
    ...
    if (num == 0) {
      ...
      $(".review-box .light").css('left', 146 + (3 * 218));
    } else {
      var len = num - 1;
      $(".review-box ul li").eq(num - 1).addClass("active").siblings().removeClass("active");
      $(".review-box .light").css('left', 146 + (len * 218));
...
function getReTime() {
  var packsNumTotal = cGet('dkdbofficial_t');
  if (packsNumTotal > 1) {
