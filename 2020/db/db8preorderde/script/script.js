...
function soldout() {
    ...
    $(".price strong").remove();
    $(".price span").html("<strong><b>26</b>,99€</strong><del>89,99€ </del>")
    $(".right h2").text("Bester Driver-Updater weltweit")
    $(".right h4").text("PC und Spiele deutlich schneller machen")
    $(".floatlayer .numbox").html('Holen Sie sich den besten Treiber-Updater mit  <strong>70% Rabatt</strong>');
    $(".floatlayer .buybtn").text('Jetzt vorbestellen');
    $(".buybtn").attr({
        "href": 'https://www.iobit.com/buy.php?product=de-db8preorder2699&ref=de_db8preordersoldout' + refStr + '&aff=' + aff + '&refs=de_purchase_db&tw=-8',
        "onclick": "ga('send', 'event', 'dbbuy', 'buy', 'db8preordersoldout-de');"
    })
    $(".banner .buybtn").html("Vorbestellen mit 70% Rabatt");
}
...
$(document).ready(function() {
    ...
    $('.review-box  span.prev').click(function(event) {
        ...
        var num = $(".review-box ul li.active").index();
        if (num == 0) {
            $(".review-box ul li").eq(1).addClass("active").siblings().removeClass("active");
            $(".review-box .light").css('left', 386 + (1 * 175));
        } else {
            var len = num - 1;
            $(".review-box ul li").eq(num - 1).addClass("active").siblings().removeClass("active");
            $(".review-box .light").css('left', 386 + (len * 175));
        }
    });
});
...
function getReTime() {
    var packsNumTotal = cGet('dedb8preorderde_t');
    if (packsNumTotal > 1) {
