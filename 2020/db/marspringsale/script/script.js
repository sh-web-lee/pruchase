...
function addUrl(name) {
	switch (name) {
		case "sd":
			$(".buybtn").attr({
				"href": "https://www.iobit.com/buy.php?product=db8sd&ref=en-db3pcsdspring" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
				"onclick": "ga('send', 'event', 'dbbuysd', 'buy', 'dbpurchase-202003')",
			});
			break;
		case "iu":
			$(".buybtn").attr({
				"href": "https://www.iobit.com/buy.php?product=db8iu&ref=en-db3pciuspring" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
				"onclick": "ga('send', 'event', 'dbbuyiu', 'buy', 'dbpurchase-202003')",
			});
			break;
		case "iusd":
			$(".buybtn").attr({
				"href": "https://www.iobit.com/buy.php?product=db8sdiu&ref=en-db3pcsdiuspring" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
...
function soldProduct() {
	...
	$(".banner .right ul").html('<li class="one"><h3>Driver Booster 8 PRO <span><b><small>$</small>23.95</b> <del>$74.85</del></span></h3><p>1 year subscription / 3 PCs</p></li>');
	...
	$(".buybtn").attr({
		"href": "https://www.iobit.com/buy.php?product=driverbooster82395&ref=en-db3pcsoldoutspring" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
		"onclick": "ga('send', 'event', 'dbbuysold', 'buy', 'dbpurchase-202003')",
	});
	$(".subscript em").text("3");
}
// soldProduct();
...
$(function() {
	...
	$(".offer .center li, .footbuy li").on("click", function() {
		...
		if (!$(this).hasClass("on")) {
			...
			if (0 < length < 4) {
				$(".banner .price .tips").remove();
				$(".banner .price").append('<div class="tips"><i class="all-icons">!</i><div class="txt"><p>The price shown in the page is valid only for this promotion.</p><p>Your Driver Booster subscription will continue after the initial subscription period at the non-promotional price of $29.99.</p><p>If you choose the Driver Booster 8 bundle pack, all products will also continue after the first year at the normal price shown in the page below.</p><p class="bottom">Cancel Auto-renewal</p><p>If you do not want to participate in the auto-renewal plan, you can uncheck the Auto-renewal box in shopping cart review page or in your account at any time before the start of the next subscription.</p></div></div>');
			...
			if (0 < length < 4) {
				...
				if (arrone === 1) {
					...
					$(".banner .price").append('<p class="only">only $1.99 / Month </p>');
					$(".buybtn").attr({
						"href": "https://www.iobit.com/buy.php?product=driverbooster82395&ref=en-db3pcspring" + refStr + "&aff=" + affStr + "&refs=purchase_db&tw=" + tw,
						"onclick": "ga('send', 'event', 'dbbuy', 'buy', 'dbpurchase-202003')",
					});
					$(".banner .price .tips").remove();
				}
			}
...
$(document).ready(function() {
	setTimeout('$(".banner .offer").addClass("on")', 500);
});
...
function timeInit() {
	var date = new Date();
	var cName = "endbmarspr_tnew" + date.getDate();
