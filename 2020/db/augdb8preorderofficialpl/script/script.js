...
function soldout() {
	...
	$(".price strong").remove();
	$(".price span").html("<strong><b>79</b>,90 zł</strong><del>299,90 zł</del>")
	$(".floatlayer .numbox").html('Zdobądź najczęściej pobierany aktualizator sterowników  <strong>70% taniej</strong>');
	$(".buybtn").attr({
		"href": 'https://www.iobit.com/buy.php?product=pl-db8preorder79&ref=pl_db8preorderofficialsoldout79' + refStr + '&aff=' + aff + '&refs=pl_purchase_db&tw=-8',
		"onclick": "ga('send', 'event', 'dbbuy', 'buy', 'db8preorderofficialsoldout-pl');"
	})
	//$(".banner .buybtn").html("Pre-order Now – 70% OFF");
}
function seview() {
	...
	if ($('.review-message .roll').length != 0) {
		...
		$('.review-box  span.prev').click(function(event) {
			...
			if(num==0) {
				...
				$(".review-box .light").css('left', 209+(2*175));
			}
			else {
				var len =num-1;
				$(".review-box ul li").eq(num-1).addClass("active").siblings().removeClass("active");
...
function getReTime() {
	var packsNumTotal = cGet('pldbofficial_t');
	if (packsNumTotal > 1) {
