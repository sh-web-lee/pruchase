...
$(document).ready(function() {
	...
	$(window).on('scroll', function() {
		...
		if ($(window).scrollTop() > $(".feature.first dd").offset().top - 200) {
			...
			setTimeout('$(".feature .curve").addClass("on");', 600);
			setTimeout(function() {
				$(".feature .curve.pink").addClass("out");
				$(".feature .rocket").addClass("on");
			}, 2400);
		}
		if ($(window).scrollTop() > $(".feature.third").offset().top - 200) {
			$(".feature.third .content").removeClass("on").addClass("on");
			setTimeout('$(".feature.third .imgbox").removeClass("on").addClass("on");', 600);
			setTimeout(function() {
				$(".feature .progressbar").removeClass("on").addClass("on");
			}, 2600);
		}
		if ($(window).scrollTop() > $(".review").offset().top - 300) {
...
function getReTime() {
	var packsNumTotal = cGet('olduserit2020_t');
	if (packsNumTotal > 1) {
		if (packs <= 3) {
			return 200000;
		}
