...
function soldout() {
    ...
    $(".buybtn").attr({
        "href": 'https://www.iobit.com/buy.php?product=fr-db8preorder2499&ref=fr_db8preorder2499' + refStr + '&aff=' + aff + '&refs=fr_purchase_db&tw=-8',
        "onclick": "ga('send', 'event',  'dbbuy', 'buy', 'db8preorder2499-fr');"
    })
    $(".banner .buybtn").html("Pre-order Now – 70% OFF");
    $(".right .buybtn").text("-65% - Vite, J'en Profite");
}
...
function getSales(isInit) {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            // console.log(getReTime());
        }
    });
}
//
function getReTime() {
    var packsNumTotal = cGet('frdb8preorderpre_t');
    if (packsNumTotal > 1) {
