...
function soldout() {
  ...
  $(".banner .numbox,.price span.one").remove();
  $(".price span.two strong").html("R$ 79,99");
  $('.price span del').css('display','block')
  $(".floatlayer .numbox").html('Obtenha o Melhor Atualizador de Drivers com <strong>60% de Desconto</strong>');
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=pt-db8preorder7999&ref=pt-db8preorderexpsoldout'+refStr+'&aff='+aff+'&refs=pt_purchase_db&tw=-8',
    "onclick": "ga('send', 'event', 'db3pcbuy', 'buy', 'db8preorderexp-pt');"
  });
  // $(".banner .buybtn").html("Adquirir Agora 60% de Desconto");
}
// soldout();
...
$(window).on('scroll', function() {
		...
		if ( $(window).scrollTop() > $(".feature .message01").offset().top-500) {
				e+=1;
				if(e==1) {
		...
		if ( $(window).scrollTop() > $(".feature .message02").offset().top-500) {
			if (flag) {
				flag = false;
              setTimeout(function() {
...
function getReTime() {
  var packsNumTotal = cGet('ptdbofficial_t');
  if (packsNumTotal > 1) {
