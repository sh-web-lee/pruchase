...
function soldout() {
    ...
    $(".price strong").remove();
    $(".price span").html("<strong><b>199</b> kr.</strong><del>679 kr.</del>")
    $(".right h2").text("Nr.1 mest downloaded opdateringsværktøj til drev")
    $(".right h4").text("Giver dig en problemfri PC og hurtig gaming")
    $(".right .buybtn").html("Forudbestil nu – 70% RABAT");
    $(".floatlayer .numbox").html('Få det mest downloaded opdateringsværktøj med <strong> 70% RABAT</strong>');
    $(".floatlayer .buybtn").text('Forudbestil nu');
    $(".buybtn").attr({
        "href": 'https://www.iobit.com/buy.php?product=dk-db8preorder199&ref=dk_db8preordersoldout' + refStr + '&aff=' + aff + '&refs=dk_purchase_db&tw=-8',
        "onclick": "ga('send', 'event', 'dbbuy', 'buy', 'db8preordersoldout-dk');"
    })
    $(".banner .buybtn").html("Forudbestil nu");
}
...
function getReTime() {
    var packsNumTotal = cGet('dkdb8preorder_t');
    if (packsNumTotal > 1) {
        if ((packsNumTotal - packs) <= 30) {
            ...
            if(packsNumTotal <= 98 && packsNumTotal >= 90){
                ...
                retime = (Math.random() > 0.5) ? 10.5 : 12.5;
            }
            return retime * 1000;
        }
    } else {
