...
function soldOut() {
	...
	$(".banner h2").html("Renew the best uninstaller with <strong>50% OFF</strong>");
	$(".floatlayer .number").html('Renew the best uninstaller with <strong> 50% OFF</strong>');
	$(".price").find("strong").html("$14.77");
	$(".buybtn").attr({
		"href": 'https://www.iobit.com/buy.php?product=iu10preorder1477&ref=en-iu10preorderexpsoldout' + refStr + '&aff=' + aff + '&refs=purchase_iu&tw=' + tw,
		"onclick": "ga('send', 'event', 'iusoldbuy', 'buy', 'iu10predorderexp');"
	});
}
// soldOut();
...
$(window).scroll(function() {
	$("[data-name^='animation']").each(function(i) {
		if ($(document).scrollTop() > $(this).offset().top - 200) {
			...
			// 			numberJump(".intro .total", 0, 1000);
			// 			numberJump(".intro .bundle", 0, 1000);
			// 			numberJump(".intro .sofeware", 0, 1000);
			// 			numberJump(".intro .plug-total", 0, 1000);
			// 			numberJump(".intro .mail", 0, 1000);
			// 			numberJump(".intro .files", 0, 1000);
			// 			numberJump(".intro .registries", 0, 1000);
			// 			numberJumpTwo(".intro .disk", 2000);
			// 		}, 300);
			// 	} else {
			// 		return;
			// 	}
...
function getReTime() {
	var packsNumTotal = cGet('eniujunexp0709_t');
