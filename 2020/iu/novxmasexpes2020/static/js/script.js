function soldOut() {
  ...
  $(".gifts .price").prepend('<p><strong>15<small>,99€</small></strong> <del>29,99€</del></p>');
  $(".footbuy .price,.gifts .price").find("del").html("29,99€");
  $(".footbuy .price").find("strong").html("15<small>,99€</small>");
  $(".buybtn").text('COMPRA YA');
...
function getCycleCountdown() {
  var date = new Date();
  if (MApp(2.2).cookie.get('novxmasexpesiu2020')) {
    dt = MApp(2.2).cookie.get('novxmasexpesiu2020');

  } else {
    dt = date.setMinutes(date.getMinutes() + 12);
    MApp(2.2).cookie.set('novxmasexpesiu2020', dt);
  }
