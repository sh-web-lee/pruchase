...
$(window).on('scroll', function() {
	var scrollHeight = $(".banner .price").offset().top;
...
function attention() {
	$(".banner .clean,.buybtn").addClass("_hide");
	$(".attention,.buybtn.attention").removeClass("_hide");
}
...
function soldOut() {
	...
	if (!$(".attention").hasClass("_hide")) {
		$(".buybtn.attention").attr({
			...
			"onclick": "ga('send', 'event', 'iusoldbuy', 'buy', 'iu10predordertrd');"
		}).html("<span>Get It Now - 50% OFF</span>");
	} else {
		$(".buybtn").attr({
			"href": 'https://www.iobit.com/buy.php?product=iu10preorder1477&ref=en-iu10preorderrcsoldout' + refStr + '&aff=' + aff + '&refs=purchase_iu&tw=' + tw,
			"onclick": "ga('send', 'event', 'iusoldbuy', 'buy', 'iu10predordertrdclean');"
		}).html("<span>Get It Now - 50% OFF</span>");
	}
}
...
function getSales() {
	...
	$.ajax({
		type: "GET",
		url: "iu10preorder.php?st=autoclean",
...
function getReTime() {
	var packsNumTotal = cGet('eniu10auto_t');
