...
function soldOut(num) {
	...
	$(".float .number p").html("La oferta especial de 47% de descuento ha terminado.")
	$(".banner .number p").html("La oferta especial de 47% <br> de descuento ha terminado.")
	//$(".buybtn").not('.buybtn-pop').html("<p>Activate Now & Save 70% </p> Don't Miss Again!");
	$(".price b").html("17");
	$(".countdown").removeClass("twinkle");

	var paramTo = MApp(2.2).url.getParameters('to');
	$(".buybtn").not('.buybtn-pop').attr("href", "https://www.iobit.com/buy.php?product=es-iu113pc1799&ref=es_iu113pcmonitor2108" + refStr + "&refs=es_purchase_iu&tw=" + tw);
	$(".buybtn").not('.buybtn-pop').attr("onclick", "ga('send', 'event', 'iubuy', 'buy', 'iu1799monitor2108-es');");
...
function packsInit() {
	var date = new Date();
	var cName = "esiumon" + date.getDate();
