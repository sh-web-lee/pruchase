...
function soldOut() {
  ...
  $(".banner h1").html("Willkommen zu unserem Vorverkauf")
  $(".banner h2").html("Erwerben Sie die Pro-Edition für 18,99€")
  //float
  $(".floatlayer .number").html('Kaufen Sie den besten Uninstaller mit <strong> 52% Rabatt </strong>');
  //price
  $(".price").find("strong").html("<del>39,95€</del> <span>18,99€</span>");
  setTimeout(function() {
    $(".price strong").addClass("remove");
  }, 500);
  //buybtn
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=de-iu10preorder1899&ref=de_iu10preorderrcsoldout' + refStr + '&aff=' + aff + '&refs=de_purchase_iu&tw=' + tw,
    "onclick": "ga('send', 'event', 'iubuy', 'buy', 'iu10predorderrcsoldout-de');"
  }).html("<span> Jetzt 52% sparen</span>");
}
...
function getSales() {
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      if (packs > 0) {
        if (packs > 50) {
          reTime = (Math.random() > 0.5) ? 1 : 3;
        } else if (packs <= 10) {
          reTime = (Math.random() > 0.5) ? 4 : 6;
        }else{
...
function getReTime() {
  var packsNumTotal = cGet('deiujunerc_t');
