...
$(window).on('scroll', function() {
	...
	var scrollHeightt = $(".banner .buybtn").offset().top;
...
function soldOut(num) {
	...
	$(".banner,.float,.buybtn").addClass("soldout");
	$(".number p").html("La réduction de 60% est terminée ! <p class='zoomout'>La réduction de 60% est terminée !</p>")
	$(".buybtn").not('.buybtn-pop').html("<p>J’en profite et j’économise 50% </p>");
	$(".price b").html("16");
	$(".countdown").removeClass("twinkle");
	$(".buybtn").not('.buybtn-pop').attr("href", "https://www.iobit.com/buy.php?product=fr-iu111y3pc1699&ref=fr_iu11soldoutinstallmonitor" + refStr + "&aff=" + aff + "&refs=fr_purchase_iu&tw=" + tw);
	$(".buybtn").not('.buybtn-pop').attr("onclick", "ga('send', 'event', 'iubuy', 'buy', 'iumonitorsoldoutpurchase-fr');");
}
...
function packsInit() {
	var date = new Date();
	var cName = "friumon" + date.getDate();
