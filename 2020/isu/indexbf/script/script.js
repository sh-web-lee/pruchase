...
function soldOut() {
	...
	$(".banner .right-message h2").html("<span>Sorry, gifts are already sold out.</span> However, you still have a chance to get <strong>55% OFF</strong> on")
	$(".banner .right-message .progress").html("<h3>IObit Software Updater 3 PRO</h3> <p>1 Year Subscription / 3 PCs</p>")
	$(".banner .right-message ul span:eq(0)").html("$29.99")
	$(".banner .right-message ul span:eq(1)").html("55% OFF")
	$(".banner .right-message ul span:eq(2)").html("$17.02")
	$(".floatlayer .number").html("<h2>Update your programs <br>Cover security holes now</h2>");
	$(".floatlayer .price del").html("$29.99")
	$(".feature .price span >span").remove();
	$(".floatlayer .wrapper").append("<p class='year'>1 Year / 3 PCs</p>");

	$(".buybtn").attr({
		"href": 'https://www.iobit.com/buy.php?product=isu33pc&ref=en-isu3pcsoldout' + refStr + '&aff=' + aff + '&refs=purchase_isu&tw=' + tw,
		"onclick": "ga('send', 'event', 'isu3pcbuy', 'buy', 'isupurchase');  "
	});
...
function getReTime() {
	var packsNumTotal = cGet('isublack2020_t');
	if (packsNumTotal > 1) {
