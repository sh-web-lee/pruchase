...
function soldOut() {
	...
	$(".banner .right-message .number").html("Ops! O desconto de 75% na pré-venda não está mais <br> disponível, mas você ainda pode desfrutar de 45% OFF.")
	$(".floatlayer .number").html("Obtenha 45% de Desconto ");
	$(".floatlayer .left-img,.banner .left-img").remove();
	$(".price del").html("R$ 99,99")
	$(".buybtn").attr({
		"href": 'https://www.iobit.com/buy.php?product=pt-isu33pc5199&ref=pt-isu3preordersoldoutofficial' + refStr + '&aff=' + aff + '&refs=pt_purchase_isu&tw=' + tw,
		"onclick": "ga('send', 'event', 'isusoldbuy', 'buy', 'isu3preorder-pt'); "
	});
}
...
$('.review .prev').click(function(event) {
	var continer = $(".review  .content").width();
	$('.review .roll').stop(true, true).animate({marginLeft: -continer}, 'slow',function(){
...
$('.feature .message-list li').hover(function(event) {
	...
	if( numbox != num ) {
		...
		$('.feature .img-box .box').stop(true, true).animate({marginTop: 0}, 'slow');
	}
});
