...
function soldOut() {
  ...
  $(".banner .right-message h2").html(
    "<span>Przepraszamy, prezenty są już wyprzedane.</span> Jednak nadal masz szansę na <strong>55% ZNIŻKI</strong> na"
  );
  $(".banner .right-message .progress").html(
    "<h3>IObit Software Updater 3 PRO</h3> <p>Roczna subskrypcja / 3 PC</p>"
  );
  ...
  $(".floatlayer .wrapper").append(
    "<p class='year'>Roczna subskrypcja / 3 PC</p>"
  );

  $(".buybtn").attr({
    href:
      "http://www.iobit.com/buy.php?product=pl-isu3pc49&ref=pl_isu3pc49purchase1911" +
      refStr +
      "&aff=" +
      aff +
      "&refs=pl_purchase_isu&tw=" +
      tw,
    onclick: "ga('send', 'event', 'isubuy', 'buy', 'isupurchase3PC2011-pl');",
  });
}
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (sales.packsNum != packs) {
        ...
        if (
          packs <= 0 &&
          !$(".banner,.gift-message,.floatlayer,.feature .price").hasClass(
            "soldout"
          )
        ) {
          soldOut();
