...
function soldOut() {
	...
	$(".banner .right-message h2").html("<span>Spiacenti, i regali gratuiti sono esauriti.</span>Non perdere l’ultima opportunità per avere il <strong>65% di sconto!</strong>");
	$(".banner .right-message .progress").html("<h3>IObit Software Updater 3 PRO</h3> <p>1 Anno / 3 PC</p>");
	$(".banner .right-message ul span:eq(0)").html("39,99€");
	$(".banner .right-message ul span:eq(1)").html("65%");
	$(".banner .right-message ul span:eq(2)").html("27€");
	$(".floatlayer .number").html("<h2>Aggiorna i tuoi programmi Ripara ora le falle nella sicurezza</h2>");
	$(".floatlayer .price del").html("39,99€");
	$(".feature .price span >span").remove();
	$(".floatlayer .wrapper").append("<p class='year'>1 Anno / 3 PC</p>");

	$(".buybtn").attr({
		
		"href": 'http://www.iobit.com/buy.php?product=it-isu33pc1299&ref=it_isu33pc1299purchasexmas2020' + refStr + '&aff=' + aff + '&refs=it_purchase_isu&tw=' + tw,
		"onclick": "ga('send', 'event', 'isubuy', 'buy', 'isupurchase3pcxmas2020-it');"
	});
}
// soldOut();
...
function getReTime() {
	var packsNumTotal = cGet('isuxmasde_t');
	if (packsNumTotal > 1) {
