...
var countdown = function countdown(_countdown) {
  getCycleCountdown();
  $(_countdown).eq(0).html(h).end().eq(1).html(i).end().eq(2).html(s);
  if (d != 0 || h != 0 || i != 0 || s != 0) setTimeout("countdown('" + _countdown + "')", 1000);else soldOut();
};
...
function getCountdown(t) {
  ...
  if (n > 0) {
    ...
    return 10 > h ? h = "00" + h : 100 > h && (h = "0" + h), Array(s, a, o, u, h);
  }
...
function soldOut() {
  ...
  $(".large.message .buybtn.large").attr({
    href: babuyhref,
    onclick: babuyoncl
  });
  $(".message-box-eaux").html("<p>-55%</p>");
  $(".largeolsa em").text("110");
  $(".largeold em").text("199,00");
  $(".message-tips").hide(); //bottom-buy
...
var review = function review(index) {
...
(function () {
  ...
  packs = 79;
  $(".packsNum").html(packs);
  setTimeout("getPacksNum()", 5000); //getPacksNum();
  //countdown
