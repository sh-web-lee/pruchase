...
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('novxmasxres2020')) {
    dt = MApp(2.2).cookie.get('novxmasxres2020');

  }else{
    dt=date.setMinutes(date.getMinutes()+12);
    MApp(2.2).cookie.set('novxmasxres2020', dt);
  }
...
function getPacksNum(){
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('novxmasxres2020')/1000;
...
function soldOut() {
  ...
  $(".gifts-store,.bottom-buy ul li.mid,.gifts,.banner .message.large").addClass("soldout");
...
(function () {
  ...
  //feature
  $(".feature .more").target($(".comparison"));
