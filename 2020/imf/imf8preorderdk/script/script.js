...
function soldOut() {
  ...
  setTimeout(function() {
    $(".reducebar").html('0 kr. forhandlet tilbud udsolgt');
    $(".banner .barbox .numbox").remove();
  }, 600);
  $(".price strong").html('199 kr.');
  $(".price sup").remove();
  $(".floatlayer h2").html('66% rabat');
  $(".floatlayer .numbox").html('Lås PRo-funktionen op for omfattende beskyttelse');
  $(".buybtn").attr({
    "href": "https://www.iobit.com/buy.php?product=dk-imf8preorder199&ref=dk_imf8preordersoldout-" + refStr + "&aff=" + aff + "&refs=dk_purchase_imf&tw=" + tw,
    "onclick": "ga('send', 'event', 'imfbuy', 'buy', 'imf8predordersoldout-dk');",
  });
}
// soldOut();

function getNumber() {
  num = cGet("marimf8preorderdk");
  if (num == null) {
