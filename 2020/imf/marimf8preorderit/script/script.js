...
function soldOut() {
  ...
  setTimeout(function() {
    $(".reducebar").html('I preordini con 0€ sono esauriti oggi');
    $(".banner .barbox .numbox").remove();
  }, 600);
  $(".price strong").html('19,99€');
  $(".price sup").remove();
  $(".floatlayer h2").html('Preordina ora con il 65% di sconto');
  $(".floatlayer .numbox").html('Sblocca la versione PRO per ottenere una protezione completa ora');
  $(".buybtn").attr({
    "href": "https://www.iobit.com/buy.php?product=it-imf8preorder3pc1999&ref=it_imf8preorder3pc1999-" + refStr + "&aff=" + aff + "&refs=it_purchase_imf&tw=" + tw,
    "onclick": "ga('send', 'event', 'imfbuy', 'buy', 'imf8preorder1999-it');",
  });
}
// soldOut();

function getNumber() {
  num = cGet("marimf8preorderit");
  if (num == null) {
