...
function webpage() {
  ...
  webpage.to('.webpage', 0.6, {
    opacity: 1,
    scale: 1,
    transformOrigin: "0% 100%",
    onComplete: function() {
      redcircle.from('.red-circle', 0.6, {
        opacity: 0,
      }, "-=0.1").to('.red-circle', 0.6, {
        opacity: 1,
        repeat: -1,
      }, "-=0.1");
      thief();
...
function safebox() {
  ...
  safe.to('.safebox', 0.8, {
    ...
    onComplete: function() {
      ...
      setTimeout('$(".thief-hook").remove()', 1800);
      TweenLite.to('.password', 0.8, {
        delay: 1.4,
        rotation: 0,
        y: 0,
...
function soldOut() {
  ...
  setTimeout(function() {
    $(".reducebar").html('Épuisés pour aujourd’hui');
    $(".banner .barbox .numbox").remove();
  }, 600);
  $(".price strong").html('23,99€');
  $(".price sup").remove();
  $(".floatlayer h2").html('Précommandez-le avec 70% de réduction');
  $(".floatlayer .numbox").html('Passez à la version PRO pour acquérir la protection complète');
  $(".buybtn").attr({
    "href": "https://www.iobit.com/buy.php?product=fr-imf8preorder2399&ref=fr_imf8preordersoldout2399-" + refStr + "&aff=" + aff + "&refs=fr_purchase_imf&tw=" + tw,
    "onclick": "ga('send', 'event', 'imfsoldbuy', 'buy', 'imf8preorder-fr');  ",
  });
}
// soldOut();

function getNumber() {
  num = cGet("marimf8preorderfr");
  if (num == null) {
