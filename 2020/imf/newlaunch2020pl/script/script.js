...
var countdown = function (countdown){
  ...
  if(d!=0||h!=0||i!=0||s!=0||mi!=0) setTimeout("countdown('"+ countdown +"')", 1);
  else  soldOut();
};
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('plimfeaster')) {
    dt = MApp(2.2).cookie.get('plimfeaster');

  }else{
    dt=date.setMinutes(date.getMinutes()+10);
    MApp(2.2).cookie.set('plimfeaster', dt);
  }
...
function getPacksNum(){
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('plimfeaster')/1000;
...
function soldOut() {
  ...
  $(".banner .wrapper >p").html("Prezenty niedostępne, skorzystaj jeszcze z wyjątkowej oferty");
  $(".gifts h2").html("Prezenty niedostępne, skorzystaj ze specjalnej oferty!");
  //banner
  $(".banner .panel .message").eq(1).addClass("soldout").children().css({"visibility": "hidden"});
  //gifts
  $(".gifts").addClass("soldout");
  $(".gifts .content").html("");
  $(".gifts .buybtn").attr({"href": 'javascript::', "onclick": ''});
  $(".comparison .buybtn").attr({"href": $imf3pc.attr("href"), "onclick": $imf3pc.attr("onclick")});
  //bottom-buy
...
(function () {
  ...
  //intro
  $(window).scrollWindow($(".feature >img"),$('.intro .ball,.intro h2,.intro .wrapper >img,.intro dl'),("on"));
  //bottom-buy
