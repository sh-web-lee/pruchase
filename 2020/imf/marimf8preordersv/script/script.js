...
function soldOut() {
  ...
  setTimeout(function() {
    $(".reducebar").html('inte tillgänglig idag');
    $(".banner .barbox .numbox").remove();
  }, 600);
  $(".price strong").html('319kr');
  $(".price sup").remove();
  $(".floatlayer h2").html('Förbeställ nu med 54% RABATT');
  $(".floatlayer .numbox").html('Lås upp den PRO utgåvan för att få fullt skydd nu');
  $(".buybtn").attr({
    "href": "https://www.iobit.com/buy.php?product=sv-imf8preorder319&ref=sv_imf8preordersoldout319-" + refStr + "&aff=" + aff + "&refs=sv_purchase_imf&tw=" + tw,
    "onclick": "ga('send', 'event', 'imfsoldbuy', 'buy', 'imf8predorder-sv');",
  });
}
// soldOut();

function getNumber() {
  num = cGet("marimf8preordersv");
  if (num == null) {
