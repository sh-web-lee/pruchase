...
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('xmasxr2020it')) {
    dt = MApp(2.2).cookie.get('xmasxr2020it');

  }else{
    dt=date.setMinutes(date.getMinutes()+15);
    MApp(2.2).cookie.set('xmasxr2020it', dt);
  }
...
function getPacksNum(){
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('xmasxr2020it')/1000;
...
function soldOut() {
  ...
  $(".gifts-store,.bottom-buy ul li.mid,.gifts,.xmas-giveaway,.messagecad,.bottom-buy ul").addClass("soldout");
