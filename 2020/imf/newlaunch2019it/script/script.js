...
function timeInit() {
  var date = new Date();
  var cName = "newlaunch2020it" + date.getDate();
...
function getPacksWithGift() {
  ...
  var packsNow;
  if (timeDiff >= 1730) {
    packsNow = 67 - Math.floor((1800 - timeDiff) / 10);
  } else {
    packsNow = Math.ceil(timeDiff / 28.9) > 0 ? Math.ceil(timeDiff / 28.9) : 0;
  }
  if (statusCheck() == 1) {
    if (packsNumGift != packsNow) {
      packsNumGift = packsNow;
      $('.packsNum-gift').html(packsNumGift);
      getUserInfo();
    }
    setTimeout('getPacksWithGift()', (packsNow > 60) ? 10000 : 28900);
  }
}
