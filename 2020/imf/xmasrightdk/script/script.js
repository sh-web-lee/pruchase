...
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('xmasfr2020')) {
    dt = MApp(2.2).cookie.get('xmasfr2020');

  }else{
    dt=date.setMinutes(date.getMinutes()+1440);
    MApp(2.2).cookie.set('xmasfr2020', dt);
  }
...
function getPacksNum(){
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('xmasfr2020')/1000;
...
function soldOut() {
  var babuyhref = $(".message3 .buybtn").attr("href"),
    ...
    // console.log(babuyoncl)
    // var soldoutlargehref = "https://www.iobit.com/buy.php?product=dk-imf83pc199&ref=dk_imf83pcpurchase2012xr&refs=dk_purchase_imf";
    // var soldoutlargega ="ga('send', 'event', 'imfbuy', 'buy', 'imf3pcsinglepurchase2012xr-dk')"
  $(".large.message .buybtn.large").attr({"href": babuyhref, "onclick": babuyoncl});
  // $("#soldoutsmall").attr({"href": soldoutlargehref, "onclick": soldoutlargega});
  // $(".message-box-eaux").html('<p>66%</p>');
  // 
  // $(".banner-tips-soldout").show();
  // $(".banner-tips").hide();
  $(".detailinfo").html('1 år / 3 PC’er + <del class="messagecad">PRO-gave</del>')
  $(".panel .two .marksoldout").html('199')
  $(".message-box-eaux").html('<p>-66%</p>');
  // 
  $(".largeolsa em").html("400");
  $(".largeold em").html("599");
  $(".message-tips").hide();
  // 
  // $(".banner .panel .boundlebg").html('<div class="marksoldoutbg"></div>')
  // $(".banner .panel .boundlebg").addClass("soldoutLargebg");
  //bottom-buy
  $(".bottom-buy ul li").eq(1).unbind("click").addClass("soldout");
  $(".gifts-store,.bottom-buy ul li.mid,.gifts,.xmas-giveaway,.messagecad").addClass("soldout");
