...
function soldOut() {
  ...
  setTimeout(function() {
    $(".reducebar").html('indisponível hoje');
    $(".banner .barbox .numbox").remove();
  }, 600);
  $(".price strong").html('R$ 69,99');
  $(".price sup").remove();
  $(".floatlayer h2").html('Adquira na Pré-Venda com 55% OFF').css({
    "fontSize": "22px",
    "lineHeight": "42px"
  });
  $(".floatlayer .numbox").html('Ative a versão PRO para obter proteção total agora');
  $(".buybtn").attr({
    "href": "https://www.iobit.com/buy.php?product=pt-imf83pc6999&ref=pt-imf8preordersoldout-" + refStr + "&refs=pt_purchase_imf&tw=" + tw,
    "onclick": "ga('send', 'event', 'imfsoldbuy', 'buy', 'imf8predorder-pt');",
  });
}

function getNumber() {
  num = cGet("marimf8preorderpt");
  if (num == null) {
