...
function soldOut() {
  var babuyhref = $(".message.small .buybtn").attr("href"),
	  babuyoncl =  $(".message.small .buybtn").attr("onclick");
	$(".large.message .buybtn.large").attr({"href": babuyhref, "onclick": babuyoncl});
  $(".largeolsa em").html("58");
  $(".largeold em").html("699");
  $(".message-tips").hide();
  //bottom-buy
  $(".bottom-buy ul li").eq(1).unbind("click").addClass("soldout");
  $(".message.large,.messagecad,.gifts-store,.gifts").addClass("soldout");
...
function bottomBuy(bot) {
  ...
  botnum = bot;
  var message = {
    "price": $panel.eq(0).find('.price').find('b').text(),
    "del":  $panel.eq(0).find('.price').find('del').text(),
    "href": $panel.eq(0).find('.buybtn').attr('href'),
    "ga": $panel.eq(0).find('.buybtn').attr('onclick')
  };
