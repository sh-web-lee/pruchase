...
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('frimfeaster')) {
    dt = MApp(2.2).cookie.get('frimfeaster');

  }else{
    dt=date.setMinutes(date.getMinutes()+10);
    MApp(2.2).cookie.set('frimfeaster', dt);
  }
...
function progressbar(len) {
  ...
  $(".reduce").addClass("on");
  setTimeout('$(".reduce").removeClass("on")', 800);
}

function getPacksNum(){
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('frimfeaster')/1000;
  var n2 = Math.floor(e.getTime()/1000);
  if(n1-n2>0){
    packs = Math.floor((n1-n2)/7.5);
    $('.soldNum').html(1000-packs);
  }else {
    packs = 0;
    $('.soldNum').html("1 000");
  }
...
function soldOut() {
  ...
  //comparison
  $(".comparison h3 span").text("70% de réduction");
