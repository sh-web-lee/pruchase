...
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('novxmasexpes2020')) {
    dt = MApp(2.2).cookie.get('novxmasexpes2020');

  }else{
    dt=date.setMinutes(date.getMinutes()+12);
    MApp(2.2).cookie.set('novxmasexpes2020', dt);
  }
...
function getPacksNum(){
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('novxmasexpes2020')/1000;
...
function soldOut() {
  ...
  $(".bottom-buy ul li.mid,.banner .message.large").addClass("soldout");
  $(".message-tips").hide();
  bottomBuy(0);
}
