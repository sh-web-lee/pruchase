...
function soldOut() {
  ...
  setTimeout(function() {
    $(".reducebar").html('heute nicht verfügbar');
    $(".banner .barbox .numbox").remove();
  }, 600);
  $(".price strong").html('21,99€');
  $(".price sup").remove();
  $(".floatlayer h2").html('Jetzt vorbestellen mit 63% Rabatt');
  $(".floatlayer .numbox").html('Schalten Sie PRO-Edition frei um den vollen Schutz zu erhalten');
  $(".buybtn").attr({
    "href": "https://www.iobit.com/buy.php?product=de-imf8preorder2199&ref=de_imf8preordersoldout" + refStr + "&aff=" + aff + "&refs=de_purchase_imf&tw=" + tw,
    "onclick": "ga('send', 'event', 'imfsoldbuy', 'buy', 'imf8predordersoldout-de');",
  });
}
// soldOut();

function getNumber() {
  num = cGet("marimf8preorderde");
  if (num == null) {
