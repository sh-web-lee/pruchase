...
var bannerObject = [{
  className:'one',
  body:'<tr><td>Driver Booster 8 PRO</td><td>3 PCs</td><td>$74.85</td><td>$0</td></tr><tr><td>IObit Software Updater 3 PRO</td><td>3 PCs</td><td>$32.99</td><td>$9.99</td></tr><tr><td>IObit Malware Fighter 8 PRO</td><td>3 PCs</td><td>$54.95</td><td>$16.99</td></tr>',
  Discount:'70% OFF + Free Gift',
  Subtotal:'$162.79',
  Total:'$26.98',
  
  url:'https://www.iobit.com/buy.php?product=2020dbimfisu2698&ref=br_dbimfisu'+refStr+'&refs=purchase_3bundlebf&tw='+tw,
  ga:"ga('send', 'event', 'dbimfisubuy', 'buy', '3bundlepurchasebf-dbimfisu');"
},
{
  className:'two',
  ...
  ga:"ga('send', 'event', 'dbbuy', 'buy', '3bundlepurchasebf-dbimfisu');",
  soldoutUrlGa:"ga('send', 'event', 'dbsoldbuy', 'buy', '3bundlepurchasebf-dbimfisu');"
},
{
  className:'three',
  ...
  ga:"ga('send', 'event', 'imfbuy', 'buy', '3bundlepurchasebf-dbimfisu');",
  soldoutUrlGa:"ga('send', 'event', 'imfsoldbuy', 'buy', '3bundlepurchasebf-dbimfisu');"
},
{
  className:'four',
  ...
  ga:"ga('send', 'event', 'isubuy', 'buy', '3bundlepurchasebf-dbimfisu');",
  soldoutUrlGa:"ga('send', 'event', 'isusoldbuy', 'buy', '3bundlepurchasebf-dbimfisu');"
}]
...
function getSales() {
  ...
  $.ajax({
    type: "GET",
    url: "bthreeb.php?pop=br_dbimfisu",
    ...
    success: function (res) {
      ...
      if (packs > 0) {
        $(window).unload(function () {
          packs = cGet('endbimfisu_p');
        });
        var total = cGet('endbimfisu_t');
...
function getReTime() {
  var packsNumTotal = cGet('endbimfisu_t');
  if (packsNumTotal > 1) {
