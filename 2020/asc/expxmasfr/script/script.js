...
function getCycleCountdown () {
  var date=new Date();
  if (MApp(2.2).cookie.get('expxmasfr')) {
    dt = MApp(2.2).cookie.get('expxmasfr');

  }else{
    dt=date.setMinutes(date.getMinutes()+12);
    MApp(2.2).cookie.set('expxmasfr', dt);
  }
...
function getPacksNum(){
  var e = new Date;
  var n1 = MApp(2.2).cookie.get('expxmasfr')/1000;
...
(function () {
  ...
  setTimeout('getPacksNum()', 5000);
  //getPacksNum();
  $(window).scrollWindow($(".intro"), $('.intro dl'), ("on"), 300);
  //countdown
