...
function soldOut() {
	...
	$(".banner .buybtn,.between-buy .buybtn,.comparison .buybtn,.floatlayer .buybtn").attr("href", "https://www.iobit.com/buy.php?product=asc143pc1999&ref=en-asc3pcsoldoutspringb" + refStr + "&aff=" + aff + "&refs=purchase_asc&tw=" + tw).attr("onclick", "ga('send', 'event', 'asc1999soldbuy', 'buy', 'ascpurchase-2020springb');");
}
...
function timeInit() {
	var date = new Date();
	var cName = "endbbstate_t" + date.getDate();
	var arr = document.cookie.match(new RegExp("(^| )" + cName + "=([^;]*)(;|$)"));
	var nowTime = Math.floor(date.getTime() / 1000);
	if (arr == null) {
		countTime = nowTime + 300;
		cSet("endbbstate_t", countTime);
	} else {
	...
	if (cGet('enascbanni_s') == -1) {
		maskTimeInit();
	} else {
		var maskStatus = cGet('enascbanni_s');
