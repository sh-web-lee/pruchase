...
function soldOut() {
  ...
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=nl-asc14preorder1999&ref=nl_asc14preorderofficial1999' + refStr + '&aff=' + aff + '&refs=nl_purchase_asc&tw=-8',
    "onclick": "ga('send', 'event', 'ascbuy', 'buy', 'asc14preorderofficialsoldout-nl');"
  });
  $(".right-message > p").html("");
}
// soldOut();
...
function getReTime() {
  var packsNumTotal = cGet('nlascoff_t');
  if (packsNumTotal > 1) {
...
$(window).on('scroll', function() {
  ...
  if ($(window).scrollTop() > $(".feature03").offset().top - 500) {
    ...
    if (c == 1) {
      setTimeout(function() {
        $(".feature03 .list01 .line").animate({
          height: '59px'
        }, 200);
        $(".feature03 .list02 .line").animate({
          height: '48px'
        }, 200);
        $(".feature03 .list03 .line").animate({
          height: '43px'
        }, 200);
      }, 500);
      setTimeout(function() {
