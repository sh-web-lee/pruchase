...
function timeInit() {
	var date = new Date();
	var cName = "enascbstate_t" + date.getDate();
	var arr = document.cookie.match(new RegExp("(^| )" + cName + "=([^;]*)(;|$)"));
	var nowTime = Math.floor(date.getTime() / 1000);
	if (arr == null) {
		countTime = nowTime + 300;
		cSet("enascbstate_t", countTime);
	} else {
	...
	if (cGet('enascblack2020_s') == -1) {
		maskTimeInit();
	} else {
		var maskStatus = cGet('enascblack2020_s');
