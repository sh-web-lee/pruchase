...
function soldOut() {
  ...
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=de-asc14preorder2499&ref=de_asc14preorderofficialsoldout'+refStr+'&aff=' + aff + '&refs=de_purchase_asc&tw=-8',
    "onclick": "ga('send', 'event', 'ascbuy', 'buy', 'asc14preorderofficialsoldout-de');"
  });
}
...
function getReTime() {
  var packsNumTotal = cGet('deascoff_t');
  if (packsNumTotal > 1) {
...
$(function() {
  packsInit(1);
  setTimeout('getViewNum()', 12000);
  $('._review_title').eq(0).hide();
  $('._review_Media_title').eq(0).show();
  $(".review  .change span").click(function(event) {
    ...
    if(num==2){
      ...
      $('._review_title').eq(0).show();
      $('._review_Media_title').eq(0).hide();
    }
