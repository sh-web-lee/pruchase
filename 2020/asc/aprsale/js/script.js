$(document).ready(function() {
	...
	setTimeout('$(".banner .message").addClass("on")', 800);
...
$(document).ready(function() {
 var number = $(".asc-message .note b:eq(0)").text();
 var number1 = $(".asc-message .note b:eq(1)").text();
 var number2 = $(".asc-message .note b:eq(2)").text();
 $(".banner .title strong").html(number*1+number1*1+number2*1);
 if ($('.banner .title strong').text()==0) {
		$(".banner .title strong").remove();
	}
});
...
function soldOut() {
	...
	$(".activate,.floatlayer,.float-center").removeClass("sold-out").addClass("sold-out");
	$(".floatlayer .img-box").find("img").attr("src", imgSrc);
	...
	$(".banner .buybtn,.between-buy .buybtn,.comparison .buybtn,.floatlayer .buybtn").attr("href", "https://www.iobit.com/buy.php?product=asc133pc1999&ref=en-asc3pcsoldoutmar" + refStr + "&aff=" + aff + "&refs=purchase_asc&tw=" + tw).attr("onclick", "ga('send', 'event', 'asc1999soldbuy', 'buy', 'ascpurchase-2020mar');");
}
