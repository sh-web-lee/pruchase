...
function soldOut() {
  ...
  $(".price a").html("Jetzt Upgraden")
  $(".banner h1").html(soldout_banner)
  $(".banner .numbox").remove();
  $(".floatlayer .numbox").html(soldout_float)
  $(".buybtn").attr({
    "href": 'https://www.iobit.com/buy.php?product=de-asc14preorder2499&ref=de_asc14preorderrcsoldout' + refStr + '&aff=' + aff + '&refs=de_purchase_asc&tw=-8',
    "onclick": "ga('send', 'event', 'asc3pcbuy', 'buy', 'asc14predorderrc-de');"
  });
}
...
function getReTime() {
  var packsNumTotal = cGet('deascrc_t');
  if (packsNumTotal > 1) {
...
$(function() {
  ...
  $(".review  .change span").click(function(event) {
	...
	if(num == 2 ){
		$(".review>h2").html('Media Review');
	}else{
		$(".review>h2").html('Kundenbewertung');
	}
  });
});
var c =0;
