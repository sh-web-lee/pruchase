...
$(function() {
  ...
  $('.footbuy li').on('click', function() {
    ...
    $('.footbuy .price a').attr({'href': dataUrl, "onclick": dataGa});
  });
});
...
function maskTimeInit() {
  if ((cGet('esiujune_s') == -1) && (!$('.popbg,.popbox').hasClass('on'))) {
    popShow();
    var date = new Date();
    var cName = 'esiujune_c';
    var arr = cGet('esiujune_c');
