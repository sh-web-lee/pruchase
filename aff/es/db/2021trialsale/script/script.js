...
function bannerSet() {
  ...
  switch (paramTo) {
    ...
    case "trialbuy_2":
      if (driverCount > 1) {
        paragraph1 = "<b class='nums'>"+driverCount + "</b> drivers disponibles para actualizar";
        paragraph2 = "Cómpralo ya para actualizar todos con 1 solo clic";
      } else {
        paragraph1 = "<b class='nums'>1</b> driver disponible para actualizar";
        paragraph2 = "Cómpralo ya para actualizarlo con 1 solo clic";
      }
      status=1;
      break;
    case "trialbuy_3":
    ...
    case "trialbuy_4":
      trialOver();
      paragraph1 = "¡Tu periodo de prueba de Driver Booster ha finalizado!";
      if (driverCount > 0) {
        if (driverCount == 1) {
          paragraph2="1 driver desactualizados. Cómpralo ya para actualizar con 1 clic";
        } else {
          paragraph2 = driverCount + " drivers desactualizados. Cómpralo ya para actualizar con 1 clic";
        }
      }else {
        paragraph2 = "Cómpralo ya para mantener tus drivers actualizados";
      }
      break;
    case "trialbuy_8":
    case "trialbuy_10":
      paragraph1="¡Tu periodo de prueba finalizará en <span class='days'>7</span> <span class='dayStr'>días</span>!";
      paragraph2="No te pierdas esta oferta única para los usuarios de la versión de prueba";
      break;
    default:
      break;
  }
...
function compSet(){
  ...
  if(leftDays>0){
    ...
    if((paramTo=='trialbuy_2')||(paramTo=='trialbuy_7')){
      $(".comparison .changed.itemc").html("Ilimitado");
      $(".comparison .exTxt.first").text("2 al día");
    }else{
      $(".comparison .changed.itemc").html("<i class='all-icons red'>√</i>");
    }
  }else{
