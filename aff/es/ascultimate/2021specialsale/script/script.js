...
function scanStart() {
  start
    .set(".banner .sd,.banner .iu,.banner .pf", {
      opacity: 0,
      scale: 0.5,
...
function soldOut() {
  ...
  $(".banner .offer.big").append(
    '<div class="mask"><span class="sold-word">TERMINATO</span></div>'
  );
  ...
  $(".footbuy .tag").html("<span>1 año /</span> 3 PC");
  $(".footbuy .price").html(
    "<strong>22.95<small>€</small></strong> <del>49,99€</del>"
  );
...
//
// function getTime() {
//   var time = cGet("2021specialsalees");
//   if (time == null) {
//     time = Time(10);
//     cSet("2021specialsalees", time);
//   }
//
//   if (Time() <= time) {
...
function getCycleCountdown() {
  var date = new Date();
  if (MApp(2.2).cookie.get("2021specialsalees")) {
    dt = MApp(2.2).cookie.get("2021specialsalees");
  } else {
    dt = date.setMinutes(date.getMinutes() + 15);
    MApp(2.2).cookie.set("2021specialsalees", dt);
  }
...
function getCountdown(t) {
  ...
  if (n > 0) {
    ...
    return (
      10 > h ? (h = "00" + h) : 100 > h && (h = "0" + h), Array(s, a, o, u, h)
    );
  }
  return 0;
}
