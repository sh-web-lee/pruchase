...
function soldOut() {
    TweenMax.to($('.cut-box'), 3, {
        ...
        onComplete: function() {
            ...
            var refLink = MApp(2.2).url.getParameters('ref');
            if (refLink.length <= 0) refLink = 'enaff_asc11_2018newyearsale';
            var tw = '&tw=' + MApp(2.2).datetime.getTimeZone();
            var refGet = MApp(2.2).url.getParameters('pop');
            if (refGet == 'b_asc') {
                refLink = refGet;
            }
            ...
            $(".buy-btn").attr("href", "https://www.iobit.com/buy.php?product=enaff-asc113pc2198&ref=" + refLink + "&aff=" + aff + "&refs=purchase_asc" + tw).attr('onclick', "ga('send', 'event', 'ascbuy', 'buy', 'enaffasc112018newyearsale');clickCount(4)");
            $("#floatlayer,.gotop").hide();
            refCheck();
            if(refLink=='fs1702en'){
                $('.buy-btn').attr('href','https://iobit.onfastspring.com/advanced-systemcare-10-pro-1-year-subscription-3-pcs-2198');
            }
        }
    });
}
...
$(function() {
    ...
    if (refGet == 'b_asc') {
        $('.buy-link').each(function() {
            ...
            var buyHref = buyHrefTemp.replace(/ref=(.*)&aff=/, 'ref=' + refGet + '&aff=');
...
function refCheck() {
    ...
    var checkArr = ['x_asc', 'xr_dbpasc', 'xr_sdpasc'];
...
function fsCheck(){
    ...
    if(ref=='fs1702en'){
        $('.link-gift').attr('href','https://iobit.onfastspring.com/advanced-systemcare-pro-with-3-free-gifts');
        $('.link-gift-pop').attr('href','https://iobit.onfastspring.com/advanced-systemcare-pro-with-3-free-gifts-1999');
    }
}