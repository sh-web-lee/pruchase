...
function soldOut() {
	...
	$(".banner .buybtn,.between-buy .buybtn,.comparison .buybtn,.floatlayer .buybtn").attr("href", "https://www.iobit.com/buy.php?product=enaff-asc133pc1999&ref=" +refGet+ refStr + "&aff=" + aff + "&tw=" + tw);
}
...
function timeInit() {
	...
	if (cGet('enaffascfes_s') == -1) {
		maskTimeInit();
	} else {
		var maskStatus = cGet('enaffascfes_s');
...
function maskTimeInit() {
	if (cGet('enaffascfes_s') == -1) {
		...
		var date = new Date();
		var cName = "enaffascfes_c";
		var arr = cGet("enaffascfes_c");
		var nowTime = Math.floor(date.getTime() / 1000);
		if (arr == null) {
