...
if (!empty($_GET['action']) && (($_GET['action']) == 'sendGift')) {
    ...
    if (isEmail($sendEmail)) {
        try {
            ...
            //mail check
            $checkSql = "select * from affnewyear2020 where sendEmail=:sendEmail and status=1";
            $pre = $pdo->prepare($checkSql);
            $pre->bindParam(':sendEmail', $sendEmail);
            $pre->execute();
            $insertSql = "insert into affnewyear2020(sendEmail,ip,addTime,status) VALUES (:sendEmail,:ip,:addTime,0)";
            ...
            if (is_numeric($return)) {
                $pdo->exec('update affnewyear2020 set status=1 where id=' . $insertId);
