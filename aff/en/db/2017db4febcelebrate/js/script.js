...
function setNumber(number){
	...
	for(var i=0;i<len;++i){
		...
		obj.animate({ backgroundPositionY:y+"px" }, 1200);
		if(len==1) {
			$(".box span").eq(2).animate({ backgroundPositionY:"-440px" }, 1200);
		}
		if(len==2) {
			$(".box span").eq(1).animate({ backgroundPositionY:"-440px" }, 1200);
		}
		if(len==3) {
			$(".box span").eq(0).animate({ backgroundPositionY:"-440px" }, 1200);
		}
	}
	setTimeout(function () {
