...
function notecard() {
  $(".mask").addClass("show");
  mask.addClass("notecard").removeClass("claim").removeClass("congrats");
  mask.html('<dl><dt>Note:</dt><dd>Thank you for your visiting and your interest in IObit products! This Special Offer is over now.</dd><dd>You will be redirected to the latest IObit promotion page automatically.</dd></dl>');
  hidenotecard();
}
...
function refCheck() {
  ...
  if ((ref.length > 0)||(aff.length>0)) {
    if (ref == 'fs1702en') {
      $('.bfp1').attr('href', '#').attr('data-fsc-action', 'Add,Checkout').attr('data-fsc-item-path-value', 'iobit-xmas-special-pack-1-pc');
      $('.bfp3').attr('href', '#').attr('data-fsc-action', 'Add,Checkout').attr('data-fsc-item-path-value', 'iobit-xmas-special-pack-3-pcs');
