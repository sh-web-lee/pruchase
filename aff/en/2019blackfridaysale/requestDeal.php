...
if (!empty($_GET['action']) && (($_GET['action']) == 'sendGift')) {
    ...
    if (isEmail($sendEmail)) {
        try {
            ...
            //mail check
            $checkSql = "select * from affblackfriday2019 where sendEmail=:sendEmail and status=1";
            $pre = $pdo->prepare($checkSql);
            $pre->bindParam(':sendEmail', $sendEmail);
            $pre->execute();
            $insertSql = "insert into affblackfriday2019(sendEmail,ip,addTime,status) VALUES (:sendEmail,:ip,:addTime,0)";
            ...
            if (is_numeric($return)) {
                $pdo->exec('update affblackfriday2019 set status=1 where id=' . $insertId);
