...
if (!empty($_GET['action']) && (($_GET['action']) == 'sendGift')) {
    ...
    if (isEmail($sendEmail)) {
        try {
            ...
            //mail check
            $checkSql = "select * from affblackfriday where sendEmail=:sendEmail and status=1";
            $pre = $pdo->prepare($checkSql);
            $pre->bindParam(':sendEmail', $sendEmail);
            $pre->execute();
            $insertSql = "insert into affblackfriday(sendEmail,ip,addTime,status,page) VALUES (:sendEmail,:ip,:addTime,0,1)";
            ...
            if (is_numeric($return)) {
                $pdo->exec('update affblackfriday set status=1 where id=' . $insertId);
