...
function timeSet(){
	...
	$(".curvebox .four").html("Nach dem "+afterDaysArr[0][1]+' '+afterDaysArr[0][0]);
	$(".curvebox .one").html("Vor dem "+before3DaysArr[0][1]+' '+before3DaysArr[0][0]);
	if(leftDays==0){
...
function getDateStr(daysArr,isBef){
	...
	if(tempArr.length==1){
		if(isBef){
			return daysArr[0][1] + ' bis ' + daysArr[1][1]+' '+daysArr[0][0];
		}else {
			return tempArr[0][1] + ' ' + tempArr[0][0];
		}
	}else{
		if(isBef){
			return daysArr[0][1] + ' ' + daysArr[0][0] + ' bis ' + daysArr[1][1] + ' ' + daysArr[1][0];
		}else {
			return daysArr[0][1] + ' ' + daysArr[0][0] + ' bis ' + daysArr[3][1] + ' ' + daysArr[3][0];
		}
	}
